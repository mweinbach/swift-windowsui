...
- `RenderFrame` is still the default renderer-neutral command list. The experimental batch path uses typed scene primitives, keeps replayable scene paint records plus per-layer family paint operations as bridge/replay metadata, carries semantic content masks on those primitives, assigns bounds-based draw orders from masked bounds inside `GPUIScene`, finishes layers into ordered batch ranges before upload, uses device-pixel scaling, a runtime-owned logical text layout cache, subtree layout/measurement reuse plus runtime-owned prepaint dispatch state and split deferred-subtree prepaint/deferred paint replay in `RetainedViewRuntime` for interaction/focus/late-paint metadata and ancestor routing, and a cached native glyph atlas with pixel-atlas fallback for icon glyphs.
...
- Remember that `PixelText.swift` / `PixelFontAtlas.swift` remain the icon/private-use fallback and the legacy frame-text path. The experimental batch scene path now prefers runtime-cached native layout plus cached glyph bitmaps, and the retained runtime now reuses cached subtree measurement/layout plus frame/scene paint ranges, runtime-owned prepaint dispatch state, and split deferred-subtree prepaint/deferred paint replay for interaction/focus/late-paint metadata and ancestor routing, but there is still no full shaped-text system, per-deferred prepaint replay range, or GPUI-style sprite family split.
...
- `Sources/SwiftWindowsUI/ScenePainter.swift`: `ViewNode` to `GPUIScene` translation plus draw-order-friendly scene emission
