...
- Remember that `PixelText.swift` / `PixelFontAtlas.swift` remain the icon/private-use fallback and the legacy frame-text path. The experimental batch scene path now prefers runtime-cached native layout plus cached glyph bitmaps, and the retained runtime now reuses cached subtree measurement/layout plus frame/scene paint ranges, but there is still no full shaped-text system, window-owned deferred draw replay pass, or GPUI-style sprite family split.
