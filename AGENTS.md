...
- Remember that `PixelText.swift` / `PixelFontAtlas.swift` remain the icon/private-use fallback and the legacy frame-text path. The experimental batch scene path now prefers runtime-cached native layout plus cached glyph bitmaps, and the retained runtime now reuses cached subtree measurement/layout plus frame/scene paint ranges and runtime-owned prepaint dispatch state for interaction/focus/deferred metadata and ancestor routing, but there is still no full shaped-text system, full deferred element replay pass, or GPUI-style sprite family split.
