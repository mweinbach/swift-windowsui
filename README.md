...
- Shared-source support: `CGFloat`/`CGPoint`/`CGSize`/`CGRect` aliases and minimal `Binding` / `State` / `Environment` / `EnvironmentValues` including `displayScale` and `pixelLength` / `ObservableObject` / `Published` / `ObservedObject` / `StateObject`
