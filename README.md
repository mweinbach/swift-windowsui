...
- Compatibility helpers: `Color(red:green:blue:opacity:)`, common non-channel named `Color` values, `Color.opacity(_:)`, `Font.system(_:design:weight:)`, `Font.custom(_:size:)`, `Font.custom(_:fixedSize:)`, `Material`, `LinearGradient(colors:startPoint:endPoint)`, `UnitPoint`, `Angle`, `FillStyle`, `DragGesture`
- Shared-source support: `CGFloat`/`CGPoint`/`CGSize`/`CGRect`, `LocalizedStringKey`/`LocalizedStringResource` aliases, and minimal `State` / `StateObject` / `Binding` / `ObservableObject` / `Published` / `ObservedObject`
