...
- Shared-source support: `CGFloat`/`CGPoint`/`CGSize`/`CGRect`, `LocalizedStringKey`/`LocalizedStringResource` aliases, `StringProtocol` title overloads, and minimal `State` / `StateObject` / `Binding` / `ObservableObject` / `Published` / `ObservedObject`
