...
- Shared-source support: `CGFloat`/`CGPoint`/`CGSize`/`CGRect` aliases and minimal `Binding` / `State` / `Environment` / `EnvironmentValues` including scene phase, control active appearance, edit mode, size classes, display scale, pixel length, calendar/time zone/locale, dismiss, refresh, window actions, undo manager, and accessibility preference values / `ObservableObject` / `Published` / `ObservedObject` / `StateObject`
