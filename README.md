...
- Core views: `Text`, `Image(systemName:)`, `Label` with system-image and custom title/icon builder forms, `LabeledContent`, `ControlGroup`, `Spacer`, `Divider`, `Rectangle`, `RoundedRectangle`, `Circle`, `Ellipse`, `Capsule`, `Group`, `GeometryReader`
...
- Text entry includes prompt-aware, click/caret-aware single-line fields, keyboard and pointer-drag selection replacement/deletion, injectable clipboard shortcuts with a Win32 clipboard adapter for hosts that install it, and a basic multiline `TextEditor`, but rich editing is still future work
