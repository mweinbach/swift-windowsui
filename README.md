...
- Shared-source support: `CGFloat`/`CGPoint`/`CGSize`/`CGRect` aliases and minimal `Binding` / `State` / `Environment` / `EnvironmentValues` including display scale, pixel length, and accessibility preference values / `ObservableObject` / `Published` / `ObservedObject` / `StateObject`
