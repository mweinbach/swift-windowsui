...
- Shared-source support: `CGFloat`/`CGPoint`/`CGSize`/`CGRect` aliases and minimal `Binding` / `State` / `Environment` / `EnvironmentValues` including scene phase, display scale, pixel length, calendar/time zone, and accessibility preference values / `ObservableObject` / `Published` / `ObservedObject` / `StateObject`
