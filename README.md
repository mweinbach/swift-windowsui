...
- Shared-source support: `CGFloat`/`CGPoint`/`CGSize`/`CGRect` aliases and minimal `Binding` / `State` / `Environment` / `EnvironmentValues` including scene phase, control active appearance, display scale, pixel length, calendar/time zone/locale, dismiss, and accessibility preference values / `ObservableObject` / `Published` / `ObservedObject` / `StateObject`
