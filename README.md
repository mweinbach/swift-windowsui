...
- Containers: `VStack`, `HStack`, `ZStack`, `ForEach`, `ScrollView`, `List`, `Form`, `GroupBox`, `Section` with header/footer builders, `HSplitView`, `VSplitView`
- Controls: `Button` with role, `systemImage`, and common style variants, `TextField`, `SecureField`, `Toggle`, stepped `Slider`, labeled `ProgressView`, `Picker` with hashable selection tags
