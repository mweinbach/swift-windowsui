...
package final class RetainedListNavigationReceipt {
    ...
    package func traceKeyboardFocus(_ stage: StaticString) {
        runtime?.traceListKeyboardFocus(stage)
    }
    ...
    package func finishNavigation() -> Bool {
        ...
        let prepared: Bool
        if let keyboardPreparation {
            traceKeyboardFocus("finish.route.cold")
            prepared = runtime.prepareLazyListKeyboardHandoff(keyboardPreparation, target: targetNode, receipt: self)
        } else {
            traceKeyboardFocus("finish.route.ordinary")
            prepared = runtime.prepareListNavigationTarget(targetNode, receipt: self)
        }
        traceKeyboardFocus(prepared ? "finish.prepare.true" : "finish.prepare.false")
        diagnosticExit = "finish.prepared-continuation"
