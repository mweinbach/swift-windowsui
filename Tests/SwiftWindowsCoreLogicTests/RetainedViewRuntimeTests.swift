...
final class RetainedViewRuntimeTests: XCTestCase {
    ...
    func testMinimumFrameIntervalSwitchesBetweenFrameAndScene() async {
        await MainActor.run {
            let node = ViewNode(
                frame: Rect(x: 0, y: 0, width: 100, height: 100),
                backgroundColor: Color(red: 1, green: 0, blue: 0, alpha: 1)
            ...
            // Start with frame at t=1.0
            let firstFrame = runtime.renderFrame(at: 1.0)
            XCTAssertEqual(fillRectCommands(in: firstFrame).first?.color, Color(red: 1, green: 0, blue: 0, alpha: 1))

            // Change and try scene too soon at t=1.005
            node.backgroundColor = Color(red: 0, green: 1, blue: 0, alpha: 1)
            let throttledScene = runtime.renderScene(at: 1.005)
            
            // Should still show old color because we're throttled
            XCTAssertEqual(throttledScene.layers[0].quads[0].startR, 1, "Should be throttled to previous render time")
            XCTAssertTrue(runtime.isDirty)

            // Wait and render frame at t=1.020
            let refreshedFrame = runtime.renderFrame(at: 1.020)
            XCTAssertEqual(fillRectCommands(in: refreshedFrame).first?.color, Color(red: 0, green: 1, blue: 0, alpha: 1))
            XCTAssertFalse(runtime.isDirty)
        }
    }
