...
| **2D transforms** | Translation, uniform scale and rotation lower onto the scene contract; a `rotationEffect` card draws rotated on both backends; ancestors compose before descendants, and the pointer inverse follows | Shears, mirrors and non-uniform scales degrade to the axis-aligned bounding box; text, images, shadows and path content (`backgroundPath`, `Canvas`) inside a rotated subtree stay axis-aligned — only quads carry a rotation, so glyph and image quads draw upright and shadows and path rasters fill the rotated frame's bounding box; a rotated `.clipped()` clips to a bounding box |
