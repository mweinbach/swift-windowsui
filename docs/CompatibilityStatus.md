...
| Host loop / invalidation coalescing | **Partial** | Native input, presentation, and coalesced observed-model rebuilds have a native wake path. That path does not drain arbitrary Swift MainActor jobs; the blocking Win32 loop still has an unresolved integration gap with the installed Swift 6.3 MainActor queue. Retained task scheduling/cancellation tests do not establish native `.task` or model-owned `Task` loader progress. The proposed dashboard loader's live workflow remains unqualified. The rejected plain Foundation RunLoop experiment and its queued-Unicode failure are recorded in `goal.md` |
...
| `List` | **Partial** | Vertical scroll panel, stable row metrics, hover/selection chrome, arrow-key selection with scroll-into-view, and limited edit chrome. Lists with more than 64 direct rows virtualize **layout** through the existing retained lazy-stack window; keyboard navigation and `ScrollViewReader` reveal offscreen rows through their already-placed geometry, and deferred rows project as realizable UIA placeholders. Small lists and containers with no scroll axis preserve their eager behavior. Disabling scroll input preserves viewport virtualization and programmatic access. Like `LazyVStack`, row construction is still eager and scales with the data count. [List keyboard navigation](ListKeyboardNavigation.md) records rebuild and reentrancy guards and the remaining animated deferred-focus completion gap |
...
| Bitmap `Image.resizable()` | **Partial** | Ordinary zero-cap-inset stretch accepts finite proposals on both axes through the existing image primitive while preserving source pixels. Nonresizable images retain intrinsic sizing. Aspect-fit/fill proposal negotiation and overflow, tiling, cap-inset rendering, and native pixel conformance remain incomplete; see [bitmap sizing](BitmapImageSizing.md) |
| `AsyncImage` | **Partial** | URL work uses a detached task; retained phase publication uses `MainActor.run`. Native phase completion remains unqualified under the host-loop integration gap above; background work and UI completion are separate. Not a full network image stack |
...
| **Source builders / geometry** | Ordinary single-expression bodies and explicit-return bodies; WinSwiftUI geometry values at its drawing boundary | `View.body` does not yet inherit SwiftUI's implicit builder behavior. Co-imported Foundation CGRect/CGSize can be ambiguous or differ from WinSwiftUI's current aliases; qualified integration fixtures do not establish Foundation geometry interoperability |
...
| **Materials / blur** | True separable-Gaussian backdrop blur on both paths (D3D11: backbuffer region copy + two-pass GPU blur, same kernel as CPU); wide radii reduce through a shared downsample chain. Contained, even-origin, 1:1 plain drawing/compositing groups receive the enclosing backdrop at each occurrence, with replacement and replay rather than baked backdrop pixels. Their 25 new CPU/D3D11 tests passed in the isolated material run; joined-root validation remains pending. `.blur(radius:)` remains an **isolated** content pass, including deferred subtrees | Rotated material quads approximate; material access through content-blur, color-effect, or Canvas isolation remains unresolved, as do unsupported group mappings and native-reference parity. A blurred scroll view's *own* indicator stays sharp; an oversized blurred subtree degrades to a hard-edged backdrop blur and still draws deferred headers |
...
| Offscreen `drawingGroup` compositing | **Partial** | Ordinary groups retain the clip/area-capped bitmap cache and replay protections. Admitted material groups instead retain a current-target scene source, receive final-attempt glyph atlases, and read the current parent at each image occurrence; existing whole-scene/execution budgets reject explicitly rather than baking a stale budget fallback. The isolated material run passed 213 cases with one existing content-blur skip; joined-root validation, unsupported mappings and independent isolation remain open |
