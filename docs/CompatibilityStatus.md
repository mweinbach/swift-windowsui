...
| `AsyncImage` | **Partial** | URL work uses a detached task; retained phase publication uses `MainActor.run`. Native phase completion remains unqualified pending the host-loop gates above; background work and UI completion are separate. Not a full network image stack |
