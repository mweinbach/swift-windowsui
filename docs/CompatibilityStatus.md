...
| `DatePicker` | **Partial** | Graphical month/day selection with mounted browsing State, inherited calendar settings, range checks, retained buttons, and existing arrow increments. At `85ddfe8`, seven of eight graphical mounted tests passed; the rejected-candidate surface case and its diagnostic still failed. Clock picking and native calendar/API parity remain open ([details](GraphicalDatePicker.md)) |
| `MultiDatePicker` | **Partial, not qualified** | At `85ddfe8`, the fresh CalendarText155 selection completed 155 cases: 152 passed and three failed. Ten of eleven MultiDatePicker mounted tests passed; the rejected-candidate surface case still failed. The two public accessibility-action cases that failed at `63fd6dd` both passed in the separate CalendarPublicActions2 selection; that does not rerun the full control class. Ranges, native calendar focus, API availability, and visual parity remain open ([details](MultiDatePicker.md)) |
...
| **Blend / drawing groups** | Multiply, screen and overlay for ordinary `QuadPrimitive` and legacy `FillRectCommand` drawing across CPU and both D3D11 paths. Focused Blend49 validation at `8b54e8e` passed all 49 selected regressions before the additive change. The additive/plusLighter candidate implements saturated premultiplied color and alpha addition in the same ordinary lane; CPU transport uses the existing premultiplied bitmap format when isolated emission requires it. Backdrop materials and admitted current-target group images retain separate replacement coverage | The additive implementation and new regressions are **uncompiled and unrun** in this source candidate. Used straight-color-effect passes that may receive emission from backdrop-dependent additive blur or bound premultiplied pixels reject explicitly; this composition remains unsupported. Payload analysis also refuses otherwise representable effect chains above its 16,777,216-pixel scan budget. PNG/BMP straight-alpha export silently loses unrepresentable emission. Material-plus-blend combinations, other primitive families and remaining public modes, general drawing-group mappings, opaque/color-mode options, and complete View/group blend semantics remain open. Historical Blend49 does not qualify this candidate, the full suite, physical GPU, native/display, native reference or performance |
