...
| **Blend / drawing groups** | Source implementations of multiply, screen and overlay for ordinary `QuadPrimitive` and legacy `FillRectCommand` drawing across CPU and both D3D11 paths. Backdrop materials and admitted current-target group images keep their separate coverage replacement to preserve translucent alpha | Additive/plusLighter correctness, material-plus-blend combinations, other primitive families and remaining public modes, general drawing-group mappings, opaque/color-mode options, and complete View/group blend semantics remain open. Source and test additions do not establish execution, native-reference or performance qualification |
