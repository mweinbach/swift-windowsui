...
| `MultiDatePicker` | **Partial, not qualified** | Retained month browsing and multi-selection compiled at `bb755b5`, but its first new control test terminated on a fatal `DateComponents` set error. A follow-up preserves all internal alias candidates in an array; fresh compilation and the complete 69-method cohort remain pending. Ranges, native calendar focus, API availability, and visual parity remain open ([details](MultiDatePicker.md)) |
