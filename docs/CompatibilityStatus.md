...
| `MultiDatePicker` | **Partial, not qualified** | At `63fd6dd`, the alias-array correction compiled and the complete 70-test calendar cohort finished without the earlier fatal set error: 65 passed and five failed. Both new alias-storage tests and all seven selection tests passed. Public accessibility actions and rejected-candidate month reconstruction still fail. Ranges, native calendar focus, API availability, and visual parity remain open ([details](MultiDatePicker.md)) |
