...
| `DatePicker` | **Partial** | Graphical month/day selection with mounted browsing State, inherited calendar settings, range checks, retained buttons, and existing arrow increments. At `db417e5`, all eight graphical mounted tests and the candidate diagnostic passed in CalendarText155; the original rejected-candidate assertions are unchanged. Clock picking and native calendar/API parity remain open ([details](GraphicalDatePicker.md)) |
| `MultiDatePicker` | **Partial, not qualified** | At `db417e5`, CalendarText155 passed all 155 cases, including all eleven MultiDatePicker mounted tests and the original rejected-candidate case. The two public accessibility-action cases that failed at `63fd6dd` both passed in the separate CalendarPublicActions2 selection; that does not rerun the full control class. Ranges, native calendar focus, API availability, and visual parity remain open ([details](MultiDatePicker.md)) |
...
| `Menu` | **Partial** | Retained popup: canvas-clamped placement, scrim/Escape dismissal, focus restore, deferred layering. Primary-action menus open through the visible indicator or unmodified F4; F4 also works with hidden indicators. Body activation, Enter/Space, and default accessibility invocation retain the primary action. Native secondary accessibility access, Alt+Down/system-key handling, and long press remain unimplemented; not a native Win32 menu bar |
...
| State wrappers | **Partial** | Ordinary `@State` and `@StateObject` in custom struct views belong to their mounted identity and host, survive reconstruction and keyed reordering, and retire with their generation. StateObject invokes its factory lazily for a new owner/slot and observes even projection-only use; supplied instances intentionally remain aliases. Escaped bindings retain the last readable value but reject writes after removal or close. Private/nested struct dynamic properties install before body evaluation through typed reflection metadata. Apart from managed `@SceneStorage` below, other wrappers retain their legacy mechanisms. Metadata limits, unmounted App/Scene ownership, inactive opaque bodies, and native lifetime/transaction qualification remain open; StateObject's mutable setter and projected-self API remain nonnative extensions. See [MountedState.md](MountedState.md) |
| `@SceneStorage` | **Partial** | Coordinator-managed windows, including Settings, own independent in-memory values that survive view reconstruction and are released on host close, failed-start teardown, or host deinitialization. Escaped bindings keep the original window; after retirement they read the wrapper default and reject writes. Uncoordinated callers retain legacy string scopes, shared by default and process-lived. No persistence across application launches. See [storage lifetime details](WinSwiftUI.md#observation-model) |
...
| **Blend / drawing groups** | Multiply, screen and overlay for ordinary `QuadPrimitive` and legacy `FillRectCommand` drawing across CPU and both D3D11 paths. Focused Blend49 validation at `8b54e8e` passed all 49 selected regressions before the additive change. The additive/plusLighter implementation provides saturated premultiplied color and alpha addition in the same ordinary lane; CPU transport uses the existing premultiplied bitmap format when isolated emission requires it. Backdrop materials and admitted current-target group images retain separate replacement coverage | Focused Additive117 validation at `2903c93` passed all 117 selected methods, covering CPU reference behavior, offscreen batch WARP and the actual legacy WARP kernel; see [Testing.md](Testing.md). Used straight-color-effect passes that may receive emission from backdrop-dependent additive blur or bound premultiplied pixels reject explicitly; this composition remains unsupported. Payload analysis also refuses otherwise representable effect chains above its 16,777,216-pixel scan budget. PNG/BMP straight-alpha export silently loses unrepresentable emission. Material-plus-blend combinations, other primitive families and remaining public modes, general drawing-group mappings, opaque/color-mode options, and complete View/group blend semantics remain open. Historical Blend49 does not qualify this candidate, the full suite, physical GPU, native/display, native reference or performance |
