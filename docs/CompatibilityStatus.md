...
| **Blend / drawing groups** | Multiply, screen and overlay for ordinary `QuadPrimitive` and legacy `FillRectCommand` drawing across CPU and both D3D11 paths. Focused Blend49 validation at `8b54e8e` passed all 49 selected regressions, covering CPU reference behavior, offscreen batch WARP and the actual legacy WARP kernel. Backdrop materials and admitted current-target group images keep their separate coverage replacement to preserve translucent alpha | Additive/plusLighter correctness, material-plus-blend combinations, other primitive families and remaining public modes, general drawing-group mappings, opaque/color-mode options, and complete View/group blend semantics remain open. The focused pass does not establish full-suite, physical-GPU, native/display, native-reference or performance qualification |
