...
| **Tab band**           | Full-bleed selector bar, 40 tall, no track | macOS insets an `.automatic` tab view and draws it with the segmented control. A recessed capsule holding three chained buttons is what made a horizontal cut through the top of the window cross four tone boundaries instead of one; see "A tab bar is a selector bar". |
| **Selector bar indicator** | Cross-fades in place; does not slide | The bar is one indicator node per tab, painted `.clear` on the ones that are not selected, which is what keeps the labels from moving on a switch. A single indicator that travels between items would need shared geometry across two subtrees the reconciler treats as siblings. |
| **Chart mark width**   | Ceiling scales with the plot; leftover goes into the gap | The design spec's fixed `clamp(available / n, 12, 40)` with a fixed 8 pt gap centres the group and leaves the rest of the plot empty — ~100 pt on each side at 1280 and 324 at 1720, which is exactly the "failed to load the rest of its data" read the centring rule exists to avoid. |
| **Chart value strip**  | 20 pt of clear above the axis maximum | The value on the tallest mark sits 6 pt above its cap, and the peak of a nice-quartered axis reaches 95 % of it. With no strip the label had nowhere to go but inside the plot, and the 100 % gridline ran through the digits. |
| **Delta caret**        | Drawn by `SymbolIconVectorRenderer`, in Supplementary PUA-A | `arrowtriangle.*.fill` mapped onto the stroked chevron, and the two Segoe carets that do exist in the icon fonts are wide rounded cups at 7 pt. A codepoint outside any icon font's range is what lets this stack state the shape of its own symbol. |
| **Data table inset**   | Full bleed, content inset by the page margin (24) | The design spec's 24 pt margin plus a separate 16 pt inset puts the screen title and the first column 8 pt apart, which reads as a broken left edge whichever of the two is right. A full-bleed selected row is also what makes the table read as a table rather than as a card that lost its border. |
