...
| Nested source atlas retention | Completed color-effect sources and their ancestors share one final native atlas buffer; retained replay copies release atlas pixels recursively. Recording another small text source does not retain another intermediate 16 MiB buffer. Old returned frames may each retain a buffer, so this is not a total RAM limit. Descendants borrow the parent's GPU atlas when the version matches | `SceneAtlasLifetimeTests`, `D3D11SharedSceneAtlasTests` |
