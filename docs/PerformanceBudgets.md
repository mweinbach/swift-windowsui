...
| Glyph atlas lookup cost | 3,000 warm lookups against a full 4,096-entry cache under 0.5 s, and a 64× larger cache may not cost 8× the time — lookup is a dictionary hit, not a scan | `CacheComplexityAndReclamationTests.testWarmCacheServesThreeThousandLookupsWithinBudget`, `…testLookupCostDoesNotScaleWithCacheSize` |
...
| Text raster cache | 256 entries and 64 MiB (`TextRasterCache` defaults), whichever binds first; LRU eviction keeps both bounds invariant. `TextRasterCache.shared` is what `Controls.icon` rasterizes through, keyed by content, style **and** device scale | `PerformanceBudgetGateTests.testTextRasterCacheEnforcesEntryCountAndMemoryBudgets`, `…testTextRasterCacheEnforcesMemoryBudgetBelowEntryCap`, `CacheComplexityAndReclamationTests.testIconRasterizationIsServedFromTheSharedRasterCache` |
