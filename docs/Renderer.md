...
Current batch renderer coverage is intentionally explicit: `D3D11BatchRenderer` draws quad, shadow, glyph-atlas, and bitmap-backed image primitive batches. `GPUIScene` carries glyph and image primitive arrays; glyph primitives sample `GPUIScene.glyphAtlasResource`, while bitmap-backed `drawBitmap` commands register their `BitmapSurface` payloads in `GPUIScene.imageResources` with matching `ImagePrimitive.textureID` values. The retained runtime keeps a bounded `TextRasterCache` for native text bitmaps, then the D3D11 batch backend uploads those image resources as BGRA shader-resource views, reuses them through a bounded bitmap-content texture cache, and renders contiguous image runs by texture ID to preserve draw order. Retained text still reaches the batch path as cached bitmap images today; the glyph-atlas path is ready for callers that emit glyph primitives directly. The inspector prints supported/unsupported primitive families plus per-scene unsupported/resource counts so regressions are visible.
...
The inspector builds small retained and `WinSwiftUI` declarative trees, reports backend/text capabilities, prints `RenderFrame`, `GPUIScene`, and `ScenePainter` primitive counts, and includes bridge-coverage, path, text, blur, retained-control, `WinSwiftUI`, text-input, multi-offset scroll-stress, text-heavy scroll-stress, and clip-stack probes without opening a window. Bridge diagnostics include skipped `fillPath`, `strokePath`, `drawText`, and `applyBlur` command counts so the batch scene contract stays honest while those command families are still represented outside `GPUIScene`.
Use `--verify` to turn those probes into a smoke gate; it exits nonzero if command emission, bridge conversion/skip accounting, retained-control focusability, `WinSwiftUI` tree generation, text input, scroll culling across the sweep, text cache population, or clip resolution regresses. Add `--json` to emit the same diagnostics as structured JSON for scripts or CI dashboards.
