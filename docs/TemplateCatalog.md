...
| Document or editor | Native dialogs choose file URLs; single regular-file FileDocument export now serializes and writes bytes atomically before success. TextEditor preserves unbound caret/selection, IME, and drag state across rebuilds with current bindings. Editor undo is not registered, and native window close cannot yet defer for unsaved changes. A complete open/edit/undo/save/unsaved-change/document-window template is still required. |
