...
- Four P1 invariant suites gate Quick as well: `ScenePresentationOrderTests` (the single draw-order authority), `SharedCoverageKernelTests` (the CPU/GPU coverage kernel), `CPUGPUBlendModeContractTests` (ordinary multiply/screen/overlay, the additive candidate's independent sum, and unchanged normal behavior) and `ClipAbstractionTests` (one clip value, one space). The expanded blend selection needs fresh timing before earlier Quick runtime estimates can be reused. The remaining P1 suites (`CPURasterizerGPUModelTests`, `PathRasterizationQualityTests`, `BorderCornerArcGeometryTests`, `TextShapingPipelineTests`) stay Full-only. Keep the Quick gate under ~10 minutes: measure a candidate before promoting it.
...
- `CPUGPUBlendModeContractTests` — ordinary multiply/screen/overlay reference
  ...
  conversion, normal behavior, and mode preservation on `QuadPrimitive` and
  through `GPUISceneBridge`. The additive candidate separately changes its old
  source-over expectations to independent saturated-sum values; the three
  separable-mode assertions remain in place.
  ...
  Historical Blend49 predates the additive candidate and is not its execution
  evidence. The new `AdditiveBlendCompositingTests`,
  `AdditiveBlendModeRasterizationTests`, `AdditiveEmissionTransportTests`,
  `D3D11AdditiveBlendTests`, `D3D11LegacyAdditiveBlendTests`, and
  `AdditiveEmissionEffectAdmissionTests` cover saturated color/alpha sums,
  coverage before clamping, group opacity after clamping, ordered destination
  reads, emission through blur/crops/nested images, binding restoration and
  resource cleanup. The effect-admission tests require explicit rejection for
  potentially unrepresentable used effect chains, with positive controls for
  ...
  per-test BGRA8 quantization/filtering tolerance without changing the numeric
  blend oracle. WARP setup failures throw rather than skip or fall back.
  Material-plus-blend combinations, other primitive families and remaining
  public modes, complete View/group semantics, the rejected effect composition
  and faithful emission export remain open. No candidate result establishes
  hardware, native/display, full-suite or performance qualification.
