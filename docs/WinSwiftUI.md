...
- `RetainedViewRuntime` now prepaints deferred overlay state for late-painted fill chrome such as scroll indicators, and both the frame and scene paths plus scroll-indicator hit testing consume that shared state instead of each recomputing overlay geometry inline.
