...
- Late-painted runtime chrome now goes through shared deferred draw records with cached frame/scene replay ranges, so clean subtree reuse can replay the deferred phase instead of regenerating every late draw every frame.
