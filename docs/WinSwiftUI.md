...
- `NavigationStack` and `NavigationView` preserve `navigationTitle` / `navigationBarTitle` metadata, render lightweight retained title chrome, and support local push/pop presentation for direct `NavigationLink(destination:)` links. `NavigationPath`, path-binding initializers, `NavigationLink(value:)`, `navigationBarTitleDisplayMode`, and `navigationDestination` are accepted for source compatibility, but typed/path-driven destination routing is not implemented yet.
