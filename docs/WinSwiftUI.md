...
- `contentMargins(_:,for:)` and `contentMargins(_:_:for:)` accept SwiftUI-shaped content-margin placement metadata. Retained `ScrollView`, `List`, and scrolling `Section` nodes resolve `.automatic` and `.scrollContent` margins into stack padding; `.scrollIndicators` is stored as compatibility metadata until retained scroll indicators have independent inset geometry.
