...
- `accessibilityHidden`
- accessibility preference environment values: `accessibilityDifferentiateWithoutColor`, `accessibilityReduceMotion`, `accessibilityReduceTransparency`, and `accessibilityShowButtonShapes`
- `help`
- `tag`
- `modifier`
...
- `EdgeInsets()`
- minimal `Binding`, `State`, `Environment`, `EnvironmentValues`, `OpenURLAction`, `DismissAction`, `RefreshAction`, `OpenWindowAction`, `DismissWindowAction`, `UndoManager`, `EditMode`, `ObservableObject`, `Published`, `ObservedObject`, and `StateObject`
...
- `EnvironmentValues.scenePhase` accepts `.active`, `.inactive`, and `.background`, defaults to `.active`, and can be overridden with `.environment(\.scenePhase, ...)` for shared-source app logic. The hosted Win32 path does not yet update it from activation or background events.
- `EnvironmentValues.controlActiveState` accepts `.key`, `.active`, and `.inactive`, while `EnvironmentValues.appearsActive` provides the newer Boolean active-appearance hint. Both values are overrideable compatibility values today; the hosted Win32 path does not yet update them from key-window or activation events.
...
- `font(_:)`, `dynamicTypeSize(_:)`, `legibilityWeight(_:)`, `tint(_:)`, `accentColor(_:)`, and `controlSize(_:)` bridge into `EnvironmentValues`, so `@Environment(\.font)`, `@Environment(\.dynamicTypeSize)`, `@Environment(\.legibilityWeight)`, `@Environment(\.tint)`, `@Environment(\.controlSize)`, `.environment(\.font, ...)`, `.environment(\.dynamicTypeSize, ...)`, `.environment(\.legibilityWeight, ...)`, `.environment(\.tint, ...)`, and `.environment(\.controlSize, ...)` share the same inherited values consumed by retained text and controls. Optional `Text.font(_:)` accepts concrete fonts and resets `nil` to the retained default text font.
...
- Accessibility preference environment values for differentiating without color, reducing motion, reducing transparency, and showing button shapes can be read with `@Environment` and overridden with `.environment`. `accessibilityReduceMotion` suppresses retained `.animation(...)` state creation for affected subtrees; the other values are compatibility metadata until the retained control and rendering layers consume them visually.
...
- `DatePicker` accepts SwiftUI-shaped date, time, closed-range, and partial-range initializer labels and maps the selected `Date` into retained label/value text. It reads `EnvironmentValues.calendar` and `EnvironmentValues.timeZone` before formatting its deterministic retained value text. `EnvironmentValues.locale` is readable and overrideable for shared-source compatibility, but locale-specific `DatePicker` formatting is not implemented yet. It is display-only today: calendar popovers, text entry, locale-specific formatting, and binding writes from user interaction are not implemented yet.
...
`@Environment` can read retained-context values such as `isEnabled`, `isScrollEnabled`, `horizontalScrollIndicatorVisibility`, `verticalScrollIndicatorVisibility`, `defaultMinListHeaderHeight`, `defaultMinListRowHeight`, `headerProminence`, `badgeProminence`, `redactionReasons`, `isPrivacySensitive`, `colorScheme`, `colorSchemeContrast`, `scenePhase`, `controlActiveState`, `appearsActive`, `editMode`, `legibilityWeight`, `displayScale`, `pixelLength`, `calendar`, `timeZone`, `locale`, `dismiss`, `refresh`, `openWindow`, `dismissWindow`, `undoManager`, `accessibilityDifferentiateWithoutColor`, `accessibilityReduceMotion`, `accessibilityReduceTransparency`, `accessibilityShowButtonShapes`, `layoutDirection`, `horizontalSizeClass`, `verticalSizeClass`, `dynamicTypeSize`, `font`, `multilineTextAlignment`, `lineLimit`, `lineSpacing`, `truncationMode`, `allowsTightening`, `textCase`, `textInputAutocapitalization`, `isAutocorrectionDisabled`, `tint`, `controlSize`, `imageScale`, `labelStyle`, `toggleStyle`, and `textFieldStyle`, and app-defined `EnvironmentKey` values can be exposed through `EnvironmentValues` extensions. `environment(_:_:)` and `preferredColorScheme(_:)` override inherited values through the retained build context.
