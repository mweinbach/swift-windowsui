...
`PreferenceKey`, `preference(key:value:)`, `transformPreference(_:_:)`, and `onPreferenceChange(_:perform:)` store type-erased preference metadata on retained `ViewNode`s and reduce descendant values in retained tree order. `transformPreference` rewrites the observed subtree value before it bubbles to ancestors, and removing the last emitted preference reports the key's default value. Anchor preferences and layout-coordinate conversion for preference payloads are not implemented yet.
