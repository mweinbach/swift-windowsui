...
- `onDeleteCommand(perform:)`, `onMoveCommand(perform:)`, `onExitCommand(perform:)`, and `pageCommand(value:in:step:)` attach retained key-command handlers to the modified node. Delete handles Backspace and Forward Delete, move handles arrow keys with `MoveCommandDirection`, exit handles Escape before the runtime clears focus, and page command handles Page Up/Page Down for `BinaryInteger` bindings while preserving values that would move outside the supplied bounds. The modifier marks the node focusable and hit-testable, but command bubbling from focused descendants is still limited.
