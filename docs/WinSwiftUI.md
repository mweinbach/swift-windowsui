...
- Named `Font` styles and `Font.system(_:design:weight:)` text-style overloads are fixed point-size and weight presets before environment scaling. `dynamicTypeSize(_:)` and `EnvironmentValues.dynamicTypeSize` scale retained `Text` and text-input font sizes with SwiftUI-shaped cases from `.xSmall` through `.accessibility5`; this is a deterministic retained scale table, not a Windows system text-size subscription yet. `Font.monospaced()`, `Text.monospaced()`, and `fontDesign(_:)` resolve through the same retained font family mapping as `.system(..., design: .monospaced)`. `font(_:)` accepts `Font?`, bridges through `EnvironmentValues.font`, and `.font(nil)` resets a subtree to the retained default font.
...
- `font(_:)`, `dynamicTypeSize(_:)`, `tint(_:)`, `accentColor(_:)`, and `controlSize(_:)` bridge into `EnvironmentValues`, so `@Environment(\.font)`, `@Environment(\.dynamicTypeSize)`, `@Environment(\.tint)`, `@Environment(\.controlSize)`, `.environment(\.font, ...)`, `.environment(\.dynamicTypeSize, ...)`, `.environment(\.tint, ...)`, and `.environment(\.controlSize, ...)` share the same inherited values consumed by retained text and controls. Optional `Text.font(_:)` accepts concrete fonts and resets `nil` to the retained default text font.
