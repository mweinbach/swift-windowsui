...
- `opacity(_:)` and `hidden(_:)` map directly onto retained node paint and visibility state. `BlendMode` accepts SwiftUI-shaped blend cases and `blendMode(_:)` stores renderer-neutral retained blend metadata. `compositingGroup()` and `drawingGroup(opaque:colorMode:)` retain offscreen-compositing intent and `ColorRenderingMode` metadata for source compatibility. The mode is lowered onto `QuadPrimitive.blendMode` and onto the `RenderFrame` commands, and carried through `GPUISceneBridge` when a frame is converted to a scene. The source implementation interprets `.multiply`, `.screen`, and `.overlay` only for ordinary `QuadPrimitive` and legacy `FillRectCommand` drawing across CPU and both D3D11 paths. D3D11 reads the current destination in a shader and then uses source-over composition. Additive/plusLighter correctness, material-plus-blend combinations, other primitive families and remaining public modes, and complete View/group blend semantics remain open. These source changes are not an execution or native/display qualification. Ordinary retained groups keep their bounded CPU bitmap cache. Admitted plain material groups use the existing current-target scene path; a dependent group inside the new content-blur implementation carries foreground and replacement coverage through CPU and GPU source paths. The new path is uncompiled/unrun in this packet. Full SwiftUI grouping, opaque/color-mode semantics, unsupported mappings, and native-reference qualification remain incomplete.
