...
- `Canvas { ctx, size in ... }` paints through the default GPUIScene path
  - multistop linear-gradient path fills and strokes preserve authored inset/diagonal endpoint vectors and context transforms on CPU and D3D11 scene paths; untransformed rectangular fills keep their dominant-axis gradient-quad fast path, and legacy `RenderFrame` path shading falls back to the first stop
