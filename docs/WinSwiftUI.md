...
- `Grid` and `GridRow` map to retained vertical and horizontal stack panels. `Grid` accepts SwiftUI-shaped alignment and spacing parameters, with vertical spacing and row alignment mapped today; full SwiftUI column sizing, spanning, and grid-cell alignment semantics are not implemented yet.
