...
- `foregroundStyle` for solid `Color`, stored `ForegroundStyle`, `AnyShapeStyle`, `LinearGradient` shape fills, `Text` value styling, and primary-style multi-argument overloads
- `imageScale`
- `symbolRenderingMode`
- `symbolVariant`
...
- `ShapeStyle` and `AnyShapeStyle` bridge `Color`, `LinearGradient`, and stored `ForegroundStyle` values into the retained foreground path; materials, hierarchical slots, and custom environment-resolved shape styles are not modeled yet.
