...
- `VStack`, `HStack`, `LazyVStack`, and `LazyHStack` accept SwiftUI-style optional spacing; `nil` resolves to the current retained default spacing of `0`. `Spacer(minLength:)` maps to a retained flexible panel and applies its minimum only along the inherited stack axis, matching SwiftUI's main-axis spacer behavior in stacks. `alignmentGuide(_:computeValue:)` accepts horizontal and vertical guide call sites, evaluates the closure against lightweight retained `ViewDimensions`, stores the resulting guide metadata on the node, and retained stacks consume matching cross-axis custom guide values for placement. Custom alignment IDs and baseline alignment are not modeled yet. Lazy stacks currently map to the same retained stack panels as eager stacks. `pinnedViews` retains section header/footer intent by marking matching `Section` header/footer nodes for deferred painting so they layer above section body content; true sticky scroll-position geometry is not implemented yet.
