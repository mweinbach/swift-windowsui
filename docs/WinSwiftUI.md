...
- `Image(_:bundle:label:)`, `Image(decorative:bundle:)`, `Image(systemName:variableValue:)`, and `resizable(capInsets:resizingMode:)` are accepted for source compatibility and reuse the same retained bitmap/icon rendering paths. Image labels and decorative flags map to retained accessibility metadata; variable symbol values, resizing mode, and cap insets are stored on retained image/icon nodes as API-shape compatibility metadata until variable SF Symbol rendering and tile/nine-slice image rendering exist.
