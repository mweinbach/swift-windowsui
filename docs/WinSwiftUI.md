...
- `LocalizedStringResource` is accepted as a lightweight compatibility value on Windows toolchains where Foundation does not expose the Apple type. `Text(LocalizedStringResource)` resolves the stored resource text before creating retained text. The current retained text node stores the resolved string only; it does not keep late-bound localization metadata after build.
