...
- `opacity(_:)` and `hidden(_:)` map directly onto retained node paint and visibility state. `BlendMode` accepts SwiftUI-shaped blend cases and `blendMode(_:)` stores renderer-neutral retained blend metadata. `compositingGroup()` and `drawingGroup(opaque:colorMode:)` retain offscreen-compositing intent and `ColorRenderingMode` metadata for source compatibility. The mode is lowered onto `QuadPrimitive.blendMode` and onto the `RenderFrame` commands, and carried through `GPUISceneBridge` when a frame is converted to a scene. The implementation interprets `.multiply`, `.screen`, and `.overlay` only for ordinary `QuadPrimitive` and legacy `FillRectCommand` drawing across CPU and both D3D11 paths. The private additive/`.plusLighter` candidate implements saturated premultiplied color and alpha addition in that same lane. D3D11 reads the current destination; the three separable modes use source-over, while additive accumulates only the saturated increment with ONE/ONE factors and leaves isolated replacement coverage unchanged. CPU image transport preserves emission in the existing premultiplied format when needed; ordinary bitmap blend dispatch is unchanged. The additive source and regressions are uncompiled and unrun. Material-plus-blend combinations, other primitive families and remaining public modes, and complete View/group blend semantics remain open. Focused Blend49 validation at `8b54e8e` passed all 49 selected regressions before this additive change, covering CPU reference behavior, offscreen batch WARP and the actual legacy WARP kernel. It does not qualify the additive candidate, full suite, physical GPU, native/display or performance. Ordinary retained groups keep their bounded CPU bitmap cache. Admitted plain material groups use the existing current-target scene path; a dependent group inside the new content-blur implementation carries foreground and replacement coverage through CPU and GPU source paths. The new path is uncompiled/unrun in this packet. Full SwiftUI grouping, opaque/color-mode semantics, unsupported mappings, and native-reference qualification remain incomplete.
- `brightness(_:)`, `contrast(_:)`, `colorInvert()`, `colorMultiply(_:)`, `saturation(_:)`, `grayscale(_:)`, `hueRotation(_:)`, and `luminanceToAlpha()` apply ordered effects to an isolated composited subtree. The scene carries the child scene, extent, and effect list; CPU rendering filters that image, while D3D11 renders the child and filters it on the same GPU device without a CPU readback. Glyphs, images, paths, shadows, and nested effects participate. Contrast and saturation use 1 for identity, and luminance-to-alpha preserves source alpha while producing black RGB. BGRA8 intermediate surfaces, bounded pass sizes, and effect clamping are explicit current contracts. The additive candidate explicitly rejects used independent color-effect passes that may receive emission from backdrop-dependent additive content blur or bound premultiplied pixels, since the existing straight-color effects cannot preserve that representation. Analysis follows actual image bindings; unused declarations, safe bitmap overrides and representable ordinary additive scenes remain admitted within the scan budget. Only used premultiplied payloads are scanned, up to 16,777,216 active pixels per query with mutation-safe content caching. Exceeding that limit explicitly refuses even an otherwise representable effect chain; ordinary scenes without a used effect pass incur no payload scan. CPU output uses the existing unsupported tile and D3D11 reports a scene-content error. This boundary is uncompiled and unrun and remains a compatibility gap. PNG/BMP export converts to straight alpha and silently loses emission with color greater than alpha; tagged premultiplied transport is separate from faithful export. Native SwiftUI color-space conformance, animated effect parameters, and full surrounding group/blend semantics remain incomplete. `Shader`, `ShaderFunction`, and `ShaderLibrary` remain source-compatibility metadata, as do `colorEffect`, `distortionEffect`, and `layerEffect`; custom shader compilation and execution are not implemented.
...
- `MultiDatePicker` keeps its existing unbounded selection initializers and maps a `Binding<Set<DateComponents>>` into a retained calendar. Previous/next month buttons browse independently of selection, and day buttons toggle supported date-only aliases using the inherited calendar, time zone, and locale. Ordinary mounted state is intended to preserve a surviving occurrence's browsed month. Pointer, Tab, Enter/Space, and accessibility default actions use retained buttons and reject callbacks from replaced calendar surfaces. The alias-array correction preserves both leap-marker forms and the public Set API. At `63fd6dd`, all 70 selected calendar methods completed without the earlier fatal set error: 65 passed and five failed. Both alias-storage tests and all seven selection tests passed. The accessible-label and accessible-default-action cases still fail to invoke the expected binding writes; the MultiDatePicker and graphical DatePicker rejected-candidate cases still reconstruct the old month. The separate graphical diagnostic repeats that failure without an extra render or query. Range overloads, grid arrow navigation, native calendar focus/style behavior, and reference-platform API availability remain unqualified; see [MultiDatePicker.md](MultiDatePicker.md).
