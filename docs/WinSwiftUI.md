...
Offscreen compositing and `drawingGroup` passes record a temporary
`GPUIScene`. They must not overwrite `ViewNode.cachedScenePaintRange`, whose
ranges belong to the outer scene. Ordinary groups keep the bounded CPU bitmap
cache and non-consuming glyph-atlas snapshot. Admitted material groups instead
retain a scene source and receive the completed paint attempt's atlas. The new
material-dependent content-blur source keeps separate foreground and replacement
coverage, including its filter halo; parent pixels are resolved at each image
occurrence, never cached on the node. Structural and execution budgets remain
explicit. That new path is source-only and uncompiled/unrun here; see
[the rendering pipeline](GPURenderingPipeline.md#material-in-content-blur-foreground-and-replacement-coverage).
...
- `ScrollViewReader`
  - `ScrollViewProxy.scrollTo(_:anchor:)` for explicit and `ForEach` identities, including virtualized lazy-stack rows
  - scoped reader ownership; reusing the same reader value in multiple runtimes safely disables its ambiguous proxy
- `List`
  ...
  - deferred flat data and `List { ForEach(...) }` source projection; construction, ownership, and unqualified limits are in [DeferredListConstruction.md](DeferredListConstruction.md)
- `Form`
...
- `EditButton`
- `PasteButton`
  - typed clipboard text, URL, and Unicode `CF_HDROP` file-list payloads
- `TextField`
...
- **Precedence.** Positive explicit frame dimensions retain their authored
  sizing intent. Ordinary `preferredSize` values are *ideals*: a greedy node
  with one (a slider, a scroll panel) reports it when nothing proposes an
  extent, and fills when something does.

Fixed `.frame(width:height:)` and concrete `.containerRelativeFrame` wrappers
store dimensions in `preferredSize` and mark only their positive finite axes
with `ViewNode.fixedPreferredSizeAxes`. The intent uses a one-byte mask;
numeric values and their existing preferred-width/height animation channels
stay in the same place. An omitted, zero, negative or nonfinite dimension is
not newly marked fixed. Alignment-only and ideal/min/max frames remain
unmarked. Reconciliation copies or clears the intent on the retained node,
including geometry-reader adoption, and changing the mask invalidates layout.
Assigning the same mask does not invalidate layout.

Absolute layout uses one policy for both intrinsic folding and placement.
On each axis, a raw extent of zero with an unmarked positive finite preference
keeps the size measurement accepted. It does not restore the larger ideal or
discard an accepted greedy proposal. The accepted result is not clamped again:
local minima and `.fixedSize` can deliberately overflow the outer proposal.
Marked fixed dimensions retain the current animated preference, including
intermediate values. Authored origins and the custom `absoluteChildFrame`
placement callback retain their existing behavior; intrinsic folding does
not consult that callback.

Legacy raw-frame sizing still uses preferred-before-raw precedence. Zero is
still the automatic-size sentinel, and other nonpositive/nonfinite branches
retain their old behavior. The preexisting fixed-width/automatic-wrapping-height
mismatch and contradictory raw-frame/local-bound behavior are not repaired by
this change. Broader fixed-frame negotiation and native SwiftUI minimum sizing
remain unverified; these internal sizing rules do not establish pixel parity.
...
- Active Windows high-contrast themes contribute their real window, text,
  control, selection, disabled, and link colors through the inherited
  environment. Retained semantic foregrounds/backgrounds, control chrome,
  selection, borders, and the default accent consume that palette; explicit app
  appearance overrides still take precedence.
- `RadialGradient` and `AngularGradient` shape styles preserve authored centers,
  radii or signed angular sweeps, multistop colors, opacity, hard stops, clips,
  rounded coverage, and transforms on the CPU and native D3D11 scene paths.
  Canvas radial/conic shading and the legacy direct-frame fallback remain
  explicitly unsupported.
- Focused text inputs support grapheme-aware Ctrl+Left/Right word navigation
...
- Explicit `Text(LocalizedStringKey, tableName:bundle:comment:)` now resolves the stored key with Foundation's `Bundle.localizedString(forKey:value:table:)`. A nil bundle uses `Bundle.main`; an omitted, nil, or empty table uses `Localizable.strings`; a missing key or table preserves the original key, while an existing empty translation stays empty. The key-only and table-only convenience initializers share that lookup. `comment` remains translator context and does not enter the displayed value. Ordinary String/StringProtocol and `Text(verbatim:)` inputs remain literal. Resolution happens once during initialization: the view still stores only the resulting String, so inherited `EnvironmentValues.locale` and later locale changes do not select another translation. The existing flattened LocalizedStringKey interpolation, LocalizedStringResource, and many control-title overloads retain their earlier limits; format interpolation, plurals, catalogs, Markdown, and automatic string-literal overload conformance are not established. Twelve new tests cover real temporary bundle tables, retained strings, fallback, bundle/table isolation, styling, and literal preservation; they and eight unchanged Text regressions passed in the focused selection on `1be9295`. The default-main convenience paths have missing-key coverage, not a translated main-bundle fixture. Common title-bearing controls and titled containers still accept StringProtocol inputs such as Substring through their existing retained label paths.
...
- SwiftUI-shaped RGB, white, and HSB `Color` initializers reduce to the renderer-neutral RGBA color type used by the retained scene. The canonical RGB color-space initializer stores encoded extended sRGB: `.sRGB` preserves finite components within the Float storage range, `.sRGBLinear` applies the signed transfer curve, and `.displayP3` converts linear primaries before re-encoding. Alpha and legacy overloads are unchanged. White initializers still clamp early, and rendering still uses the existing bounded output. See [RGB conversion and numeric limits](ColorSpaceConversion.md).
...
- Ordinary `WindowGroup` declarations retain their main-actor content builder and construct fresh root values for each managed or directly hosted window. A host keeps its materialized roots through observed-object rebuilds; it does not rerun the window builder on each redraw. Scene environment values and objects wrap the builder as well as the inspectable declaration snapshot. Explicit assignment to `WindowGroupConfiguration.content` overrides the stored builder. Data-bound scenes retain their existing payload factory and Settings retains its singleton routing. Ordinary State and StateObject in these materialized views are installed per mounted identity and host; returning one preconstructed view does not merge its owned cells across windows. StateObject factories that return a supplied instance intentionally share that object, and other wrappers retain their legacy storage mechanisms. See [MountedState.md](MountedState.md) for ownership and reference-value limits.
- Independently admitted native-window factories and mounted/document root factories now evaluate their root values under an explicit root context before component construction. Ordinary environment providers are copied without being read; the original native preparation UUID or mounted root descriptor/lookup must remain current before and after the synchronous content callback. A completed retained-row admission is not reused as the new root's authority. Rejection withholds the value and uses the existing rejected-source path so that a lost owner cannot publish a false empty root; caller scope and the original post-assignment checks remain in place. Work already inside the callback may finish after revocation. Legacy factories, direct internal host entry, eager scene declaration content, and by-value snapshot argument evaluation retain their separate limitations. The new owned-root regression class supplements the unchanged Invoke tests; fresh execution and actual native-window qualification remain required.
- `EnvironmentValues.openSettings` provides a SwiftUI-shaped `OpenSettingsAction`, and `SettingsLink` maps to a retained button that calls it. Coordinator-managed apps route it to the first declared `Settings` scene, creating one independent window or restoring/showing and requesting activation of the existing window. A denied Windows foreground request does not create a duplicate. Closing and reopening creates a fresh scene-storage scope. Static `SceneBuilder` bodies collect multiple scenes and availability checks, and scene modifiers apply to every collected configuration. Settings is not automatically opened at launch; Settings-only apps and native Settings menu generation remain unsupported. Standalone hosts can still inject their own action. The demo registers a shared Settings form and uses Ctrl+, through the normal retained shortcut path, preserving modal input isolation.
...
- `EnvironmentValues.undoManager` accepts an optional `UndoManager` for shared-source command and editing code. Hosted windows install a stable per-window manager. Its main-actor stacks support `registerUndo(withTarget:handler:)`, reciprocal undo/redo registration, action names, `removeAllActions()`, target-specific removal, and nested `disableUndoRegistration()` / `enableUndoRegistration()`. Targets are weak, dead-target actions are pruned, and handlers cannot recursively replay another action while a replay request is active. A new ordinary action invalidates redo history. Global clearing resets registration disabling; target-specific clearing does not. An unmatched enable is a Windows precondition failure because Objective-C exceptions are unavailable. Captured payloads are released after both stored stacks are updated so deinitializers can safely reenter the manager. Nonsecure retained editors automatically register accepted edits and validate replay before consuming an action, with focused keyboard fallback described in [TextInputUndo.md](TextInputUndo.md). Native Edit-menu integration, grouping, and full Foundation lifecycle conformance remain open.
...
- `task(priority:_:)` schedules an async Swift task when the retained node first appears and cancels it when that retained subtree disappears. `task(id:priority:_:)` accepts SwiftUI-shaped id call sites, relaunches when rebuilt with a changed id, and cancels the previous retained lifecycle task before starting the replacement. A rebuild from `onAppear` finishes the node/task phase after its new layout settles. Closing or releasing a host revokes its editor and State writes before canceling remaining tasks, without synthesizing disappearance or focus callbacks from deinitialization.
  These are retained scheduling and cancellation semantics, exercised by
  headless tests with a cooperative executor. The native `App` path still has
  an unresolved integration gap between its blocking Win32 loop and the
  installed Swift 6.3 MainActor queue. This limits claims for `.task` and
  model-owned `Task` loaders that require that queue; it does not imply that
  all background work or native callbacks stall.
  Observed-model changes already published on the UI thread use a separate
  native invalidation/rebuild path; that path does not drain MainActor jobs.
  The proposed dashboard load/retry/cancel workflow remains unqualified in a
  live window. A native integration must preserve Unicode/native input routing.
  The plain Foundation RunLoop experiment failed the Unicode probe and has
  not been adopted.
...
- `onAppear` fires when the retained node first renders, and `onDisappear` fires when an appeared retained subtree is removed or replaced. Scene and fallback-frame rendering share one appearance/size-change stage after layout, so paint replay and renderer switching do not duplicate callbacks; layout-only queries do not deliver them. Rebuilds during delivery defer the remaining callbacks until the next resolved render, and host teardown cancels pending delivery without synthesizing disappearance from deinitialization.
- `onChange(of:initial:_:)` owns its comparison history through the host's typed mounted view identity. Hosts, siblings, conditional branches, and explicit identities no longer share history by source location. Accepted builds deliver comparisons and actions after adoption under their captured transaction; discarded candidates do not advance history. The modern zero-argument and two-argument overloads and deprecated one-argument `perform:` form remain available. A raw component without a mount coordinator does not admit observation. Native scheduling and appearance/task ordering remain unqualified; see [MountedOnChange.md](MountedOnChange.md).
...
- `onLongPressGesture` accepts the current `perform:onPressingChanged:` and deprecated `pressing:perform:` overloads. Recognition uses the runtime clock and original logical start position, honors minimum duration and maximum distance, and finishes once at its deadline or a late eligible release. Short holds, excessive movement, capture/focus loss, disabled or removed content, and competing modal scope cancel it. Reconciliation updates callbacks without restarting thresholds; terminal callbacks run after tree adoption and cannot retire a new reentrant attempt. `LongPressGesture.updating` uses the authored transaction and resets only its own GestureState update. Nonfinite or negative parameters cannot recognize. Native callback ordering, general mounted GestureState lifetime, and complete gesture arbitration remain unqualified.
...
- `contentShape(UnevenRoundedRectangle(...))` now keeps four independent circular corner radii for retained pointer hit testing. Logical leading/trailing corners resolve in the modifier's actual inherited layout direction, including reused and doubly erased `AnyShape` values and rebuilt retained owners. The runtime stores geometry values; it does not retain or call an authored path during layout or hit testing. The existing circular clamp remains: negative radii, NaN, and negative infinity become zero; oversized finite radii and positive infinity cap at half the shorter resolved extent. Equal finite radii keep the uniform predicate, including its half-open frame edges and collapsed-bounds behavior; no new invalid-coordinate policy is implied. The separate visual-clip descriptor preserves all four radii for scene clipping; uniform hover/focus effects and the legacy `retainedClipShapeStyle` projection still use a maximum radius. `InsetShape` carries only its existing radius adjustment, `TrimmedShape` forwards the underlying content shape rather than a partial-path hit region, and `containerShape` metadata does not repair `ContainerRelativeShape` parent geometry. Last-kind selection, action order, even-odd, and mask metadata are unchanged. All fifteen focused regressions passed at `8550455`; three existing content-shape compatibility tests also passed separately. These focused results do not establish full-suite, native GPU, or macOS qualification.
...
- `Image(_:)` resolves direct file paths or bundle resources through the WIC-backed image loader and maps decoded bitmaps onto retained bitmap nodes that emit `DrawBitmapCommand`/`ImagePrimitive` resources. Named loading retains installed WIC format/frame-zero behavior within explicit encoded/pixel bounds; its 64-entry/32-MiB LRU verifies opened-file identity and metadata before cache hits. Asset-catalog lookup is not implemented yet. `AsyncImage` uses mounted request ownership and an independently bounded per-host loading service, with no global URL phases or temporary-file decoding. See [AsyncImage bounds, source density, and validation limits](AsyncImageLoading.md).
- `ImageResource(name:bundle:)` is accepted as a lightweight generated-asset compatibility value. `Image(ImageResource)` and image-resource label/control overloads resolve through the same retained bitmap loading path as named images. `ColorResource(name:bundle:)`, `Color(ColorResource)`, and `Color(_:bundle:)` are accepted for generated color asset source compatibility; hex-like resource names such as `#336699`, `#33669980`, `0xF80`, and `0F08` resolve to retained colors, while ordinary asset names still use the deterministic retained fallback `Color.accentColor` until asset-catalog color lookup exists. Bitmap `Image.resizable()` without an aspect modifier accepts finite proposals independently on both axes. Resizable bitmap and generic-view `.fit`/`scaledToFit()` now share retained fit measurement for positive finite two-axis proposals with zero minima. Admitted whole-texel cap insets and bounded tile repetition use one image primitive and the original source bytes; see [bitmap sizing and its unrun source oracles](BitmapImageSizing.md). Fill, symbol resizing, unspecified/single-axis proposal negotiation, full modifier-order behavior, and native pixel/filtering parity remain incomplete. Unsupported cap/tile plans report typed failures rather than silently stretching.
- `Color.RGBColorSpace` supports `.sRGB`, `.sRGBLinear`, and `.displayP3` in the canonical RGB initializer by converting into retained encoded extended sRGB components. Extended values are retained at Float precision without unit-interval clipping; invalid RGB and overflow use an explicit Windows policy. White-initializer range handling, physical wide-gamut/HDR output, and pinned native color-space qualification remain incomplete. [Conversion scope and native observation plan](ColorSpaceConversion.md) describes those boundaries.
...
- `VStack`, `HStack`, `LazyVStack`, and `LazyHStack` accept SwiftUI-style optional spacing; `nil` resolves to `MacOSControlMetrics.Layout.defaultStackSpacing` (8pt, macOS's default adjacent-view spacing), and `spacing: 0` stays an explicit choice. Lazy grids use the same default. `Spacer(minLength:)` maps to a retained flexible panel that is *greedy* along the inherited stack axis (`layoutFillAxes`) and applies its minimum only along that axis, matching SwiftUI's main-axis spacer behavior in stacks. Greed travels: a stack holding a spacer along its own axis takes the width (or height) it is proposed rather than the extent of its content, which is what makes `HStack { Text; Spacer; Button }.frame(height:)` span its container. The chain stops at any ancestor that pins its own extent on that axis, so a `.frame(width:)` around a spacer row still measures at the author's width. `HStackLayout`, `VStackLayout`, `ZStackLayout`, and `AnyLayout` accept common SwiftUI layout-switching call sites and map back to the retained stack or absolute z-stack adapters; `ViewSpacing` and `LayoutProperties` are lightweight compatibility values for custom-layout source. `AlignmentID`, `HorizontalAlignment(_:)`, `VerticalAlignment(_:)`, and `alignmentGuide(_:computeValue:)` accept built-in and custom guide call sites, evaluate closures against lightweight retained `ViewDimensions`, store the resulting guide metadata on the node, and retained stacks consume matching cross-axis custom guide values for placement. `LayoutValueKey` and `layoutValue(key:value:)` store type-erased retained layout metadata on modified nodes for future custom-layout consumption. `ContainerValueKey`, `ContainerValues`, and `containerValue(_:_:)` accept SwiftUI's key-path based container-value call sites and store type-erased retained container metadata on modified nodes; `.tag(_:)` also records retained container tag metadata for `ContainerValues.tag(for:)` and `hasTag(_:)`. Direct-subview `SubviewsCollection` consumption is not modeled yet. `VerticalAlignment.firstTextBaseline` and `.lastTextBaseline` are accepted for `HStack`, `LazyHStack`, `GridRow`, `ViewDimensions`, and alignment-guide calls, using a deterministic retained baseline approximation until text-system-owned baseline metrics are available. Lazy stacks map to the same retained stack panels as eager stacks, but with `ViewLayoutMode.lazyStack`: rows the scroll viewport plus a full viewport of overscan cannot reach are still measured and placed — a stack cannot know where row 900 goes without knowing how tall rows 0…899 are — but their subtrees are not laid out recursively, and they project to accessibility as flagged placeholders carrying their real bounds rather than a subtree of zero-size rectangles. A lazy stack with nothing scrollable above it behaves exactly like an eager one. Ordinary lazy-stack consumers still expand all row descriptors before layout, and `onLayout` does not fire for a deferred row. ForEach now retains data and its row factory; the flat public List route consumes its metadata without that eager expansion. This does not activate deferred construction for LazyVStack, LazyHStack, or lazy grids. See [DeferredListConstruction.md](DeferredListConstruction.md) for the current source route and remaining work. Explicit static Lists with more than 64 direct rows keep the earlier layout-virtualization behavior, including when scroll input is disabled. `pinnedViews` retains section header/footer intent by marking matching `Section` header/footer nodes for deferred painting so they layer above section body content; true sticky scroll-position geometry is not implemented yet.
- `Grid` measures shared column tracks across its direct `GridRow` children, including structurally expanded Group/ForEach content. Rows share measured widths, spans consume contiguous tracks, and direct non-row children span the grid. `gridCellColumns(_:)` no longer changes `layoutPriority`; anchors, unsized flexible demand, and a single column alignment override participate in layout. `GridRow(alignment: nil)` inherits Grid's vertical alignment. This remains a **partial** implementation: default nil spacing still resolves to zero, standalone GridRow retains its legacy horizontal-stack fallback, and general span distribution, heterogeneous compression/priorities, merged custom guides, and exact native RTL behavior remain uncharacterized. See [Grid layout](GridLayout.md) for the measured subset, explicit policies, and tests.
...
- `ProgressView(value:total:)` maps into retained progress chrome for `Double` and generic `BinaryFloatingPoint` values; title, builder-label, and current-value label overloads wrap that retained indicator in small retained stacks. `ProgressView(timerInterval:countsDown:)` overloads compute retained progress from the current `Date` at build time and accept custom label/current-value label builders; automatic ticking updates and SwiftUI's default date-progress label text are not modeled yet. `progressViewStyle(_:)` now accepts `ProgressViewStyle` conformers, including the default, linear, and circular styles. Custom struct bodies and `ProgressView(configuration)` delegation passed the focused 171-case Windows run. The old fixed-value carrier is renamed `ProgressViewStyleProfile`; `EnvironmentValues.progressViewStyle` remains a Windows-only built-in fallback profile. `.automatic` and `.linear` still use the retained progress bar, while `.circular` composes the same renderer-neutral retained circular segment indicator. See [ProgressViewStyles.md](ProgressViewStyles.md) for migration, mounted ownership, and the unqualified native/generic-source boundaries.
...
An otherwise unnamed host root uses the created window's caption as its UIA
Name, without changing its retained metadata, PANE type or text capabilities.
Explicit labels, text, child behavior and selected-content roots keep their own
...
- `UnevenRoundedRectangle` accepts `RectangleCornerRadii` and per-corner radius initializers. Direct retained fill/border geometry already preserves all four radii and resolves leading/trailing through the build context. Its public `path(in:)` now builds four circular corner arcs in local coordinates, with each radius independently clamped to the shorter half-extent using the existing uniform Path constructor's rule. Equal radii preserve that constructor's element representation. Both corner styles still use circular arcs; continuous-corner fidelity and public-path RTL qualification remain open. Actual scene clipping now transports an independent original rectangle and all four circular radii through rectangular crops. The separate immutable logical clip descriptor resolves leading/trailing at the real build context; the legacy `retainedClipShapeStyle` remains its documented maximum-radius compatibility projection. Shadows, outlines and the legacy frame route retain uniform-radius limits. Bounds-dependent retained trim, `ContainerRelativeShape` parent geometry, and `InsetShape` consistency are not repaired by this path change. Nonfinite bounds retain the existing stored-coordinate behavior and are not newly promised to render safely. Thirteen path/retained-Canvas regressions passed in the focused `734c71e` repair run; that result does not qualify macOS parity or continuous corners.
...
- `clipShape(_:style:)` maps `Rectangle`, `RoundedRectangle`, `UnevenRoundedRectangle`, `Capsule`, `Circle`, `Ellipse`, `ContainerRelativeShape`, and `AnyShape` wrappers around those retained shapes to retained bounds clipping with matching retained corner-radius behavior, and preserves `FillStyle` even-odd plus antialiasing metadata for future renderer-neutral path clipping. For one circular rounded shape, scene primitives carry the original shape rectangle and four radii independently of later rectangular crops; CPU and D3D11 output masks use that original geometry, including partial corner arcs and thin clips. `AnyShape` (including double erasure), `InsetShape` and `TrimmedShape` cache or forward the immutable descriptor without retaining an authored path callback. Their metadata forwarding does not repair inset bounds or partial-trim masks. Nested rounded intersections keep the existing innermost-shape policy; continuous corners, general path masks and legacy frame rendering remain incomplete. The new transport and tests require fresh compilation and CPU/D3D11 execution; the older path/content-shape results above do not validate its changed buffer ABI. Other `Shape` conformers currently degrade to rectangular clipping until renderer-neutral path clipping grows beyond the existing render graph fallback.
...
- `opacity(_:)` and `hidden(_:)` map directly onto retained node paint and visibility state. `BlendMode` accepts SwiftUI-shaped blend cases and `blendMode(_:)` stores renderer-neutral retained blend metadata. `compositingGroup()` and `drawingGroup(opaque:colorMode:)` retain offscreen-compositing intent and `ColorRenderingMode` metadata for source compatibility. The mode is lowered onto `QuadPrimitive.blendMode` and onto the `RenderFrame` commands, and carried through `GPUISceneBridge` when a frame is converted to a scene. The implementation interprets `.multiply`, `.screen`, and `.overlay` only for ordinary `QuadPrimitive` and legacy `FillRectCommand` drawing across CPU and both D3D11 paths. The additive/`.plusLighter` implementation provides saturated premultiplied color and alpha addition in that same lane. D3D11 reads the current destination; the three separable modes use source-over, while additive accumulates only the saturated increment with ONE/ONE factors and leaves isolated replacement coverage unchanged. CPU image transport preserves emission in the existing premultiplied format when needed; ordinary bitmap blend dispatch is unchanged. Focused Additive117 validation at `2903c93` passed all 117 selected methods, covering CPU reference behavior, offscreen batch WARP and the actual legacy WARP kernel; see [Testing.md](Testing.md). Material-plus-blend combinations, other primitive families and remaining public modes, and complete View/group blend semantics remain open. Focused Blend49 validation at `8b54e8e` passed all 49 selected regressions before this additive change, covering CPU reference behavior, offscreen batch WARP and the actual legacy WARP kernel. It does not qualify the additive candidate, full suite, physical GPU, native/display or performance. Ordinary retained groups keep their bounded CPU bitmap cache. Admitted plain material groups use the existing current-target scene path; a dependent group inside the new content-blur implementation carries foreground and replacement coverage through CPU and GPU source paths. The new path is uncompiled/unrun in this packet. Full SwiftUI grouping, opaque/color-mode semantics, unsupported mappings, and native-reference qualification remain incomplete.
- `brightness(_:)`, `contrast(_:)`, `colorInvert()`, `colorMultiply(_:)`, `saturation(_:)`, `grayscale(_:)`, `hueRotation(_:)`, and `luminanceToAlpha()` apply ordered effects to an isolated composited subtree. The scene carries the child scene, extent, and effect list; CPU rendering filters that image, while D3D11 renders the child and filters it on the same GPU device without a CPU readback. Glyphs, images, paths, shadows, and nested effects participate. Contrast and saturation use 1 for identity, and luminance-to-alpha preserves source alpha while producing black RGB. BGRA8 intermediate surfaces, bounded pass sizes, and effect clamping are explicit current contracts. The additive implementation explicitly rejects used independent color-effect passes that may receive emission from backdrop-dependent additive content blur or bound premultiplied pixels, since the existing straight-color effects cannot preserve that representation. Analysis follows actual image bindings; unused declarations, safe bitmap overrides and representable ordinary additive scenes remain admitted within the scan budget. Only used premultiplied payloads are scanned, up to 16,777,216 active pixels per query with mutation-safe content caching. Exceeding that limit explicitly refuses even an otherwise representable effect chain; ordinary scenes without a used effect pass incur no payload scan. CPU output uses the existing unsupported tile and D3D11 reports a scene-content error. The explicit rejection boundary passed its focused CPU/WARP regressions in Additive117; the rejected effect composition remains a compatibility gap. PNG/BMP export converts to straight alpha and silently loses emission with color greater than alpha; tagged premultiplied transport is separate from faithful export. Native SwiftUI color-space conformance, animated effect parameters, and full surrounding group/blend semantics remain incomplete. `Shader`, `ShaderFunction`, and `ShaderLibrary` remain source-compatibility metadata, as do `colorEffect`, `distortionEffect`, and `layerEffect`; custom shader compilation and execution are not implemented.
- `zIndex(_:)`, `offset`, `scaleEffect`, `rotationEffect`, `flipsForRightToLeftLayoutDirection(_:)`, `transformEffect(_:)`, and `projectionEffect(_:)` map to retained ordering and Transform2D state. Scale and rotation overloads accept anchor arguments but currently use the node center; anchor placement remains incomplete. Ordinary unqualified planar anchors select the planar overloads, while typed UnitPoint3D anchors and explicit z parameters retain access to the optional planar adapter. Ancestors compose before descendants. Translation, uniform scale, and rotation lower onto the scene primitives, including glyphs, images, shadows, and paths; rotated clips use a bounded offscreen pass. General inherited shears, mirrors, and nonuniform scales retain a bounding-box paint fallback even though the interaction inverse and descendant placement preserve the transform algebra. Context-authored Canvas symbol affine placement has a separate explicit image basis, described in [Canvas symbols](CanvasSymbols.md); it does not remove those general View limits. The legacy frame route still lacks general rotation encoding, and an oversized rotated clip falls back to its bounding box. See [the rendering pipeline](GPURenderingPipeline.md) for coverage and fallback details. `CGAffineTransform` and `ProjectionTransform` are lightweight compatibility values backed by the retained affine transform path. `UnitPoint3D`, `RotationAxis3D`, `Rotation3D`, `AffineTransform3D`, `rotation3DEffect(...)`, `perspectiveRotationEffect(...)`, and `transform3DEffect(_:)` accept modern SwiftUI-shaped 3D call sites; retained rendering maps z-axis rotation to the existing 2D transform and stores full 3D transforms as metadata until a renderer-neutral 3D transform contract exists.
...
- `blur(radius:opaque:)` maps onto `ViewNode.contentBlurRadius`, distinct from the node's own backdrop effect (`blurRadius`, set by Material). Material-free subtrees keep the isolated bitmap cache, including deferred descendants. A subtree that reads its enclosing backdrop now records a bounded scene source with separate foreground and replacement coverage; CPU and D3D11 source implementations filter both planes and compose against the current parent at each occurrence. The source includes its full halo, while final output clipping remains separate from source-local clips. New regression tests and the historical positive-smoothing assertion are **uncompiled and unrun** here. The opaque hint stays metadata; native SwiftUI modifier-order, edge and opacity behavior remain unqualified. A blurred scroll view's own indicator stays sharp, independent color-effect/Canvas boundaries remain separate, and only the old material-free oversize route keeps the hard-edged backdrop fallback. The frame path has no blur primitive and still degrades to sharp content. See [the rendering pipeline](GPURenderingPipeline.md#material-in-content-blur-foreground-and-replacement-coverage) for the exact bounds and cache ownership.
...
- `scrollIndicatorsFlash(onAppear:)` and `scrollIndicatorsFlash(trigger:)` propagate inherited retained flash metadata to `ScrollView`, `List`, and scrolling `Section` nodes. An appearing view or changed trigger reveals the overlay scroller, and the retained runtime fades it back out; on-appear behavior follows the existing retained view lifecycle.
- `scrollPosition(_:,anchor:)` and `scrollPosition(id:anchor:)` accept SwiftUI-shaped `Binding<ScrollPosition>` and ID bindings, then store semantic position metadata on retained `ScrollView`, `List`, and scrolling `Section` nodes. `ScrollPosition` supports ID, edge, point, x, and y construction plus `scrollTo(...)` mutations for shared-source code; binding-driven runtime updates remain unwired. Programmatic `ScrollViewProxy.scrollTo` is implemented separately.
- `onScrollGeometryChange(for:of:action:)`, both `onScrollPhaseChange` overloads, and `onScrollVisibilityChange(threshold:_:)` dispatch callbacks after a complete retained paint. Geometry uses the presented offset, including keyboard-animation lag and edge overshoot, plus natural content size, insets, and container size. Derived values are deduplicated and observer history survives reconciliation. Geometry and phase select the first enclosed scroll container; multiple candidates emit a diagnostic. Visibility intersects transformed rectangular bounds with enclosing scroll clips; it does not test opacity or sibling occlusion. The first complete render delivers initial geometry with equal old/new values and an initial visibility result. Phase delivery is bounded when input outpaces rendering. This timing, Windows impulse-phase mapping, and precise threshold/rounded-mask semantics remain subject to native-reference qualification. `onScrollTargetVisibilityChange(idType:threshold:_:)` and `onScroll` still retain metadata without dispatching their callbacks.
...
- `scrollBounceBehavior(_:axes:)` propagates horizontal and vertical `ScrollBounceBehavior` environment values, defaulting to the vertical axis like SwiftUI. `.automatic`, `.always`, `.basedOnSize`, and `.never` are stored as retained metadata on `ScrollView`, `List`, and scrolling `Section` nodes. The runtime has bounded momentum and edge overshoot, but these authored modes do not yet select its physics behavior.
...
- `ScrollViewReader` provides a SwiftUI-shaped `ScrollViewProxy` whose `scrollTo(_:anchor:)` resolves explicit `.id(...)` values and implicit `ForEach` identities within that reader's own subtree. The retained runtime moves the target's nearest vertical or horizontal scroll container, revealing only the minimum required without an anchor or aligning the requested target/viewport anchor while clamping to content bounds. Off-screen virtualized `LazyVStack` / `LazyHStack` rows resolve from their placed retained frames, and requests issued before layout replay once during the first scene or frame render. Requests capture the complete transaction, including explicit animation suppression. Authored duration and easing drive the presented offset; retargeting starts there, input interrupts motion, and more precise lazy-row alignment uses the remaining time without extending the original deadline. Nested readers retain independent scopes; missing IDs remain unchanged, while `.scrollDisabled` does not block programmatic movement. True simultaneous two-axis scrolling and binding-driven `scrollPosition` updates are not implemented. Native SwiftUI animation and callback timing remain reference-qualification work.
...
- `Menu` maps to a retained menu button and custom retained popup overlay anchored below the button. Named-image and system-image label initializers reuse retained `Label` composition for bitmap or system icon labels. Menu content gets `EnvironmentValues.isPresented` and `dismiss`, and retained menu item activation automatically closes the overlay after running the action. Primary-action initializers retain the primary closure for body clicks, Enter/Space, and default accessibility invocation; clicking the visible indicator or pressing unmodified, nonrepeated F4 opens the secondary menu. F4 remains available with a hidden indicator, without adding a focus stop or overriding authored keyboard shortcuts. Native accessibility cannot yet open that secondary menu independently; Alt+Down/system-key handling and long press also remain unimplemented. `menuStyle(_:)` and `EnvironmentValues.menuStyle` accept `.automatic`, `.button`, `.borderedButton`, `.borderlessButton`, `DefaultMenuStyle`, `ButtonMenuStyle`, `BorderedButtonMenuStyle`, and `BorderlessButtonMenuStyle`; bordered styles use retained button chrome, borderless styles use retained plain button chrome, and `showsMenuIndicator` controls the disclosure glyph unless an explicit `.menuIndicator(...)` modifier overrides it. Native Win32 menu chrome, complete keyboard menu navigation, and platform menu roles are not modeled yet. The sixteen new primary/disclosure routing tests and thirteen existing Menu regressions passed on `1be9295`; these are retained routing and presentation tests, not native COM input or state lifetime proof for freshly reconstructed ComponentHost bodies.
...
- `DatePicker` accepts SwiftUI-shaped date, time, closed-range, and partial-range initializer labels and maps the selected `Date` into retained label/value text. It reads `EnvironmentValues.calendar` and `EnvironmentValues.timeZone` before formatting its deterministic retained value text, and non-current `EnvironmentValues.locale` overrides use `DateFormatter` for locale-specific date/time text. Retained date pickers are focusable and write the selection binding from arrow-key increments while respecting the supplied range. `datePickerStyle(_:)` and `EnvironmentValues.datePickerStyle` accept `.automatic`, `.compact`, `.field`, `.graphical`, `.stepperField`, `.wheel`, `DefaultDatePickerStyle`, `CompactDatePickerStyle`, `FieldDatePickerStyle`, `GraphicalDatePickerStyle`, `StepperFieldDatePickerStyle`, and `WheelDatePickerStyle`; automatic uses the default retained label/value row, compact uses a dropdown-style retained shell, field and stepper-field styles use bordered retained input chrome, wheel uses a clipped wheel-style retained shell, and graphical uses an interactive retained month/day grid for date components. Month browsing keeps independent mounted State, day selection respects the supplied date range, and calendar labels follow the inherited calendar, time zone, and locale. The time-only graphical style retains its formatted value and minute adjustments without a fake calendar. Calendar popovers, graphical clock picking, wheel columns, editable fields, direct text entry, and native calendar focus/pixel parity are not implemented yet. See [GraphicalDatePicker.md](GraphicalDatePicker.md) for the selection policies, source-test scope, and remaining native validation.
- `MultiDatePicker` keeps its existing unbounded selection initializers and maps a `Binding<Set<DateComponents>>` into a retained calendar. Previous/next month buttons browse independently of selection, and day buttons toggle supported date-only aliases using the inherited calendar, time zone, and locale. Ordinary mounted state is intended to preserve a surviving occurrence's browsed month. Pointer, Tab, Enter/Space, and accessibility default actions use retained buttons and reject callbacks from replaced calendar surfaces. The alias-array correction preserves both leap-marker forms and the public Set API. At `db417e5`, CalendarText155 passed all 155 methods, including all eleven MultiDatePicker mounted methods, all eight graphical mounted methods and the graphical diagnostic. The original three calendar failure assertions are unchanged. CalendarReader24 separately passed its three new member-retention regressions and twenty-one existing methods. The two public accessibility-action cases passed their separate two-method selection at `85ddfe8`; earlier alias-storage and selection results remain documented separately. Range overloads, grid arrow navigation, native calendar focus/style behavior, and reference-platform API availability remain unqualified; see [MultiDatePicker.md](MultiDatePicker.md).
- `ColorPicker` accepts SwiftUI-shaped title, builder-label, and `supportsOpacity` initializer labels, then maps the selected `Color` into an `NSColorWell`-shaped retained bezel and swatch. Retained color pickers are focusable and provide keyboard binding writes: left/right cycle a deterministic retained color palette, and up/down adjust opacity when `supportsOpacity` is true. `.environment(\.colorPickerUsesNativeDialog, true)` opts into the native Windows `ChooseColorW` dialog; direct channel text entry and a native AppKit color well are not implemented.
- `PasteButton` reads requested clipboard content types through `ClipboardManager`. `.fileURL` and `.url` accept complete Unicode `CF_HDROP` file lists, overlapping accepted URL/text types deliver each payload only once, and `.url` also accepts valid plain-text web URLs; `.fileURL` never treats a web URL as a local file.
- `HSplitView` and `VSplitView` map into the retained split-view control and can infer an initial ratio from content.
- `GeometryReader` reports the slot it actually occupies, resolved in two phases. It carries a node of its own — greedy on both axes (`layoutFillAxes = .both`), laying its body out absolutely — so there is always one node whose resolved frame *is* the slot; a collapsed reader would have none whenever its body pins its own extent. Phase one is the build: the proxy is seeded from the build context's canvas, which containers narrow as they consume chrome (`TabView` measures its tab band and hands the page a reduced canvas through `ViewBuildContext.withCanvasSize`). Build order cannot do better than a seed, because the reader's siblings have not been measured yet. Phase two is the runtime's: after each layout pass, `RetainedViewRuntime.resolveGeometryReaderSlots` re-invokes the body of every reader whose resolved frame differs from the size it was built against, adopts the rebuilt node onto the existing one (so the reader keeps its place, its frame, and everything the runtime hung on it) and lays out again. The loop terminates on `ViewNode.geometryReaderBuiltSize` matching the resolved slot, and is capped at four rounds so a reader under an unbounded proposal — a scroll axis, where the body's own size feeds the slot — settles instead of oscillating. Trees with no reader in them never enter the loop. Measurement remains keyed on the resolved slot rather than the identity seed. State ownership is separate: the host now opens a generation-specific descendant build scope for deferred content, preserving its mounted State while rejecting removed or closed reader closures. It reevaluates correctly after canvas-size changes; see [MountedState.md](MountedState.md) for those ownership boundaries. `GeometryProxy.size`, `safeAreaInsets`, `frame(in:)`, `CoordinateSpaceProtocol`, `GlobalCoordinateSpace`, `LocalCoordinateSpace`, `NamedCoordinateSpace`, and `bounds(of:)` are available for shared-source call sites; frame and bounds requests currently return deterministic retained canvas bounds for `.local`, `.global`, and named spaces, and safe-area insets are zero because the Win32 host renders into the client area. `GeometryProxy3D` provides matching source-compatible 3D geometry access for `visualEffect3D`, with depth defaulting to zero and transforms defaulting to identity. `coordinateSpace(_:)` and `coordinateSpace(name:)` are accepted as no-op retained metadata boundaries until full coordinate-space projection exists.
...
lifecycle. Nonsecure `TextField` and `TextEditor` now register one accepted edit
per undo action through the inherited manager, preserving the current bindings
and saved caret/selection across replay. Exact Ctrl+Z, Ctrl+Shift+Z, and Ctrl+Y
route eligible editor history after existing explicit commands. Secure inputs
do not retain automatic plaintext history. [TextInputUndo.md](TextInputUndo.md)
documents programmatic-write invalidation, the `.id(documentSessionID)` boundary
for equal-content document switches, and selective cleanup on host close.
Typing groups, vertical caret navigation, internal scrolling, multi-range
editing, and complete document workflows remain incomplete.
...
`DynamicProperty` has a default no-op `update()`. Before evaluating a custom struct view, the retained host installs supported private and nested struct declarations into a copy, then updates children before their parent. Typed reflection metadata identifies declarations; unsupported or ambiguous owning metadata rejects the candidate with a diagnostic. Trusted non-owning wrappers retain their existing mechanisms. This is an ordinary State installation path, not full wrapper or toolchain conformance; see [MountedState.md](MountedState.md).
...
StateObject calls its factory only when resolving a new owner and declaration;
rebuilding that owner does not run replacement factories. Escaped `$state` and
`$object.member` bindings remain last-value read handles but reject writes and
cannot reconnect to a replacement owner. Provisional root and GeometryReader
epochs keep construction separate from adoption, and queued requests preserve
their binding transaction while rejecting superseded State revisions.
Outside a mounted host, State retains its seed box and StateObject caches a
separate fallback object on first access. Reference values keep ordinary alias
semantics. See [MountedState.md](MountedState.md)
...
Owned presentation schedules its existing observed-object batch on the main actor even when a frame cannot be submitted. The task consumes that batch without directly requesting a second frame; the original native wake remains in place. A live legacy HWND keeps its existing blocking-loop behavior, while a headless legacy host still requests its follow-up frame. The decision uses the presentation mode selected at host construction, not whether a native owner has already been installed.

Batch relevance, the latest relevant transaction, reentrant changes and teardown still use the existing consumer. Adoption can still owe lifecycle, accessibility and dialog work; this policy does not promise zero native work or establish native queue fairness. Eight new async fixtures cover this scheduling policy and unstarted owned-host behavior, but live HWND liveness and native smoke qualification remain separate validation requirements.
...
- The complete retained UI host and Win32/D3D11 integration remain Windows-only. Core, Graphics, Layout, Scene, and their offscreen CPU foundations are portable; the shared demo builds against native Apple SwiftUI on macOS.
...
The independent calendar candidate-retention defect subsequently passed its original three regressions at `db417e5`, together with CalendarReader24 and CalendarText155; see [GraphicalDatePicker.md](GraphicalDatePicker.md).
