...
- `labelStyle(_:)` propagates through `EnvironmentValues` and maps `.automatic` / `.titleAndIcon`, `.iconOnly`, `.titleOnly`, `DefaultLabelStyle`, `TitleAndIconLabelStyle`, `IconOnlyLabelStyle`, and `TitleOnlyLabelStyle` to retained `Label` composition.
