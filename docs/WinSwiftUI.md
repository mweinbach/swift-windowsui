...
- `onAppear` fires when the retained node first renders, `onDisappear` fires when an appeared retained subtree is removed or replaced, and `onChange(of:initial:_:)` keeps lightweight call-site state so rebuilt SwiftUI-shaped views can observe `Equatable` value transitions. Scene and fallback-frame rendering share one appearance/size-change stage after layout, so paint replay and renderer switching do not duplicate callbacks; layout-only queries do not deliver them. Rebuilds during delivery defer the remaining callbacks until the next resolved render, and host teardown cancels pending delivery without synthesizing disappearance from deinitialization. The modern zero-argument and two-argument action overloads are supported, along with the deprecated one-argument `perform:` form.
