...
- `NavigationStack` and `NavigationView` preserve `navigationTitle` / `navigationBarTitle` metadata, render lightweight retained title chrome, and support local push/pop presentation for direct `NavigationLink(destination:)` links plus `NavigationLink(value:)` routes resolved by `navigationDestination(for:)`. `NavigationPath`, path-binding initializers, `navigationBarTitleDisplayMode`, and boolean/item `navigationDestination` overloads are accepted for source compatibility, but path-driven binding synchronization and modal-style destination presentation are not implemented yet.
