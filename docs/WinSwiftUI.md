...
- `popover(isPresented:attachmentAnchor:arrowEdge:content:)` maps to a retained floating glass card with a transparent dismissal layer. The common `.rect(.bounds)` attachment anchor is accepted for source compatibility; precise source-rect anchoring and arrow rendering are future work, so placement currently follows the requested `arrowEdge` within the window.
