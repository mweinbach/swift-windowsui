...
- `font(_:)`, `dynamicTypeSize(_:)`, `legibilityWeight(_:)`, `tint(_:)`, `accentColor(_:)`, `buttonRepeatBehavior(_:)`, and `controlSize(_:)` bridge into `EnvironmentValues`, so `@Environment(\.font)`, `@Environment(\.dynamicTypeSize)`, `@Environment(\.legibilityWeight)`, `@Environment(\.tint)`, `@Environment(\.buttonRepeatBehavior)`, `@Environment(\.controlSize)`, `.environment(\.font, ...)`, `.environment(\.dynamicTypeSize, ...)`, `.environment(\.legibilityWeight, ...)`, `.environment(\.tint, ...)`, `.environment(\.buttonRepeatBehavior, ...)`, and `.environment(\.controlSize, ...)` share the same inherited values consumed by retained text and controls. Optional `Text.font(_:)` accepts concrete fonts and resets `nil` to the retained default text font. `buttonRepeatBehavior` accepts `.automatic`, `.enabled`, and `.disabled` as compatibility metadata; retained buttons do not yet repeatedly invoke actions during prolonged pointer or keyboard presses.
