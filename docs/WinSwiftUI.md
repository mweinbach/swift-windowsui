...
- Ordinary `WindowGroup` declarations retain their main-actor content builder and construct fresh root values for each managed or directly hosted window. A host keeps its materialized roots through observed-object rebuilds; it does not rerun the window builder on each redraw. Scene environment values and objects wrap the builder as well as the inspectable declaration snapshot. Explicit assignment to `WindowGroupConfiguration.content` overrides the stored builder. Data-bound scenes retain their existing payload factory and Settings retains its singleton routing. Ordinary State in these materialized views is now installed per mounted identity and host; returning one preconstructed value does not merge its ordinary State cells across windows. StateObject and other legacy wrappers can still share their supplied storage. See [MountedState.md](MountedState.md) for ownership and reference-value limits.
...
This identity path now owns ordinary `@State` cells per mounted view and host.
...
Escaped `$state` bindings remain last-value read handles but reject writes and
...
their binding transaction while rejecting superseded model revisions.
Outside a mounted host, the wrapper still retains its seed box. Reference
values keep ordinary alias semantics. See [MountedState.md](MountedState.md)
for metadata requirements, teardown ordering, inactive-container limits, and
the remaining StateObject and native conformance work.
