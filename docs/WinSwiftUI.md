...
- `opacity(_:)` and `hidden(_:)` map directly onto retained node paint and visibility state. `BlendMode` accepts SwiftUI-shaped blend cases and `blendMode(_:)` stores renderer-neutral retained blend metadata. `compositingGroup()` and `drawingGroup(opaque:colorMode:)` retain offscreen-compositing intent and `ColorRenderingMode` metadata for source compatibility. The mode is lowered onto `QuadPrimitive.blendMode` and onto the `RenderFrame` commands, and carried through `GPUISceneBridge` when a frame is converted to a scene. The implementation interprets `.multiply`, `.screen`, and `.overlay` only for ordinary `QuadPrimitive` and legacy `FillRectCommand` drawing across CPU and both D3D11 paths. D3D11 reads the current destination in a shader and then uses source-over composition. Additive/plusLighter correctness, material-plus-blend combinations, other primitive families and remaining public modes, and complete View/group blend semantics remain open. Focused Blend49 validation at `8b54e8e` passed all 49 selected regressions, covering CPU reference behavior, offscreen batch WARP and the actual legacy WARP kernel. This does not establish full-suite, physical-GPU, native/display or performance qualification. Ordinary retained groups keep their bounded CPU bitmap cache. Admitted plain material groups use the existing current-target scene path; a dependent group inside the new content-blur implementation carries foreground and replacement coverage through CPU and GPU source paths. The new path is uncompiled/unrun in this packet. Full SwiftUI grouping, opaque/color-mode semantics, unsupported mappings, and native-reference qualification remain incomplete.
- `brightness(_:)`, `contrast(_:)`, `colorInvert()`, `colorMultiply(_:)`, `saturation(_:)`, `grayscale(_:)`, `hueRotation(_:)`, and `luminanceToAlpha()` apply ordered effects to an isolated composited subtree. The scene carries the child scene, extent, and effect list; CPU rendering filters that image, while D3D11 renders the child and filters it on the same GPU device without a CPU readback. Glyphs, images, paths, shadows, and nested effects participate. Contrast and saturation use 1 for identity, and luminance-to-alpha preserves source alpha while producing black RGB. BGRA8 intermediate surfaces, bounded pass sizes, and effect clamping are explicit current contracts; native SwiftUI color-space conformance, animated effect parameters, and full surrounding group/blend semantics remain incomplete. `Shader`, `ShaderFunction`, and `ShaderLibrary` remain source-compatibility metadata, as do `colorEffect`, `distortionEffect`, and `layerEffect`; custom shader compilation and execution are not implemented.
