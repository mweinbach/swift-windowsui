...
- `MultiDatePicker` keeps its existing unbounded selection initializers and maps a `Binding<Set<DateComponents>>` into a retained calendar. Previous/next month buttons browse independently of selection, and day buttons toggle supported date-only aliases using the inherited calendar, time zone, and locale. Ordinary mounted state is intended to preserve a surviving occurrence's browsed month. Pointer, Tab, Enter/Space, and accessibility default actions use retained buttons and reject callbacks from replaced calendar surfaces. The source compiled at `bb755b5`, but the first new control test terminated with a fatal `DateComponents` set error. A follow-up keeps the internal alias candidates in an array, preserving both leap-marker forms and the public Set API; fresh compilation and the complete 69-method cohort remain pending. A separate existing graphical DatePicker test also fails to preserve the expected browsed month. Range overloads, grid arrow navigation, native calendar focus/style behavior, and reference-platform API availability remain unqualified; see [MultiDatePicker.md](MultiDatePicker.md).
