...
- `VStack`, `HStack`, `LazyVStack`, and `LazyHStack` accept SwiftUI-style optional spacing; `nil` resolves to the current retained default spacing of `0`. Lazy stacks currently map to the same retained stack panels as eager stacks. `pinnedViews` retains section header/footer intent by marking matching `Section` header/footer nodes for deferred painting so they layer above section body content; true sticky scroll-position geometry is not implemented yet.
