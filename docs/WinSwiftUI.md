...
- `RetainedViewRuntime` now builds reusable prepaint dispatch state for interaction hit regions, focus order, ancestor routing, and deferred draw metadata such as scroll indicators, and both rendering plus pointer/focus/scroll/drag routing consume that shared state instead of each walking the tree separately.
- Late-painted runtime chrome now goes through shared deferred draw records with rerunnable payloads plus cached frame/scene replay ranges, so clean subtree reuse can replay the deferred phase and cross-backend fallback can still regenerate late draws when only one cached path exists.
...
- Text behavior on the default path is still bitmap-based, while the experimental scene path has a partial native glyph-atlas port with cached logical layout, subtree layout/measurement reuse, runtime-owned prepaint dispatch state plus deferred draw replay for interaction/focus/late-paint metadata and ancestor routing, semantic content masks, inherited-opacity propagation, and glyph-run capture that still stops short of GPUI-style shaped text runs, full deferred element replay, and sprite families.
