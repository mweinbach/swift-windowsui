...
- `onDeleteCommand(perform:)`, `onMoveCommand(perform:)`, and `onExitCommand(perform:)` attach retained key-command handlers to the modified node. Delete handles Backspace and Forward Delete, move handles arrow keys with `MoveCommandDirection`, and exit handles Escape before the runtime clears focus. The modifier marks the node focusable and hit-testable, but command bubbling from focused descendants is still limited.
