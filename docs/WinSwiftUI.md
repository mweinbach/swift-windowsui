...
- `UnevenRoundedRectangle` accepts `RectangleCornerRadii` and per-corner radius initializers. Direct retained fill/border geometry already preserves all four radii and resolves leading/trailing through the build context. Its public `path(in:)` now builds four circular corner arcs in local coordinates, with each radius independently clamped to the shorter half-extent using the existing uniform Path constructor's rule. Equal radii preserve that constructor's element representation. Both corner styles still use circular arcs; continuous-corner fidelity and public-path RTL qualification remain open. The separate visual clip descriptor, retained clipping approximation, and legacy uniform-radius consumers still use the maximum radius. Bounds-dependent retained trim, `ContainerRelativeShape` parent geometry, and `InsetShape` consistency are not repaired by this path change. Nonfinite bounds retain the existing stored-coordinate behavior and are not newly promised to render safely. Thirteen path/retained-Canvas regressions passed in the focused `734c71e` repair run; that result does not qualify macOS parity or continuous corners.
