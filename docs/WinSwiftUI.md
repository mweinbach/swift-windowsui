...
- `MultiDatePicker` keeps its existing unbounded selection initializers and maps a `Binding<Set<DateComponents>>` into a retained calendar. Previous/next month buttons browse independently of selection, and day buttons toggle supported date-only aliases using the inherited calendar, time zone, and locale. Ordinary mounted state preserves a surviving occurrence's browsed month. Pointer, Tab, Enter/Space, and accessibility default actions use retained buttons and reject callbacks from replaced calendar surfaces. The new source and 32 regression methods await execution. Range overloads, grid arrow navigation, native calendar focus/style behavior, and reference-platform API availability remain unqualified; see [MultiDatePicker.md](MultiDatePicker.md).
