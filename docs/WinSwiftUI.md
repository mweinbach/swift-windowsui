...
- `NavigationStack` and `NavigationView` preserve `navigationTitle` / `navigationBarTitle` metadata and render lightweight retained title chrome around the root content. `NavigationPath`, path-binding initializers, `NavigationLink` destination/value initializers, `navigationBarTitleDisplayMode`, and `navigationDestination` are accepted for source compatibility, but WinSwiftUI does not implement push/pop navigation state or destination presentation yet. `NavigationLink` renders its label content today.
