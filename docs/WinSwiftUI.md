...
- `RetainedViewRuntime` now builds reusable prepaint state for interaction hit regions, focus order, and deferred overlay chrome such as scroll indicators, and both rendering plus pointer/focus routing consume that shared state instead of each walking the tree separately.
