...
### Ninth integration: bounded window-owned media-browser template source (2026-09-02)

The recovered and independently reviewed media-browser packet is integrated over `2647bffcec96f3dcd695c7864f0bce719ac96ddc`. The shared demo's existing gallery screen now composes `DemoMediaBrowserTemplate` using a model owned by `DemoWindowState`; window destruction closes that model's service and mounted lifetime. The existing file preview workflow and search terms remain intact. This source uses ordinary shared SwiftUI-shaped List, Buttons, selection, image, task, visibility and typed drop APIs. It does not introduce a native control, renderer-specific workaround or new snapshot catalog entry.

The workflow admits at most 64 metadata records, displays four per page (up to sixteen nonempty pages), and supports stable selection, grid/list presentation, thumbnail and selected preview wells, retry, cancellation, removal/clearing/restoring samples, and typed incoming file-URL drops. An empty collection displays one empty page. The first 64 incoming URLs bound drop scanning, and lexical file identity deduplicates admitted records while preserving the original URL. This is a finite template policy, not a reduction of the original large-grid, media or whole-product requirements.

One thumbnail lane and one preview lane call the existing image service. At most four current-page thumbnail intents and one selected preview intent wait in the model; the service itself still has two physical workers and no work queue. Canceling or superseding a request revokes its publication permission without freeing its occupied lane until the actual service call returns. Page, content revision, request, mount and lifetime identities reject late publication. Error-description callbacks are followed by another original-authority check, and notifications occur after model state changes. The model keeps at most four page thumbnails and one preview separately from the existing 32-image/16-MiB service cache, transient results and renderer/codec storage. Input and source-image bounds remain 8 MiB and sixteen million pixels; thumbnail and preview edges remain 256 and 1024.

A mounted StateObject supplies one fresh mount identity per actual view lifetime. The public task waits for appearance before activating it; finite geometry-visibility callbacks create current-page demand, not row construction, onAppear alone or optional prefetch. Old mount callbacks cannot close the replacement. Visibility does not establish OS foreground, occlusion or minimized-window state. Close prevents new admission, clears model-held image state and schedules the existing actor service closure. `awaitServiceClose()` does not prove that blocked OS reads or decoder work have drained, and cancellation does not preempt those calls.

All successful image values come from the existing real decoder/service, including the owned encoded PNG sample bytes. Malformed and unsupported bytes remain failures. There is no injected decoded success, synchronous file read in a view body, source-file modification, new platform decoder, dialog, outgoing drag or periodic timer. The existing file-URL validator, image adapter and image service are byte-identical to the packet's original base at this integration point.

Three new test source files contain 25 async model methods and fourteen async retained interaction methods. They cover physical-lane occupation after cancellation, latest intent, mount/page/revision isolation, bounded drop admission, notifications/reentry, error/close behavior, independent window ownership, actual decoded sample dimensions/pixels, public Buttons and keyboard List selection, typed drop, inner/outer clipping, page replacement, removal/remount and responsive composition. Button activation requires exactly one actual enabled focusable public Button and verifies focus before Space; it does not fall back to an ancestor action. All 661 pre-existing test files, including the new collapsed-axis controls, remain unchanged.

The retained fixtures explicitly rebuild their hosts and use no-op observation registration, so these methods do not establish automatic host ObservedObject delivery. Private appearance-waiter suspension/replacement/cancellation branches remain source-reviewed rather than directly driven by these Windows cases. The responsive case resizes already decoded content, not an in-flight load. Cleanup cancels/releases controlled readers but does not universally join every lifecycle/service task or establish deallocation. Retained keyboard/drop calls and BitmapSurface inspection do not prove Explorer/OLE, Narrator, a composited PNG, macOS behavior, native presentation or color/metadata fidelity. Those gaps remain part of the original completion work.

Root verified all 27 sealed packet payloads, the six unchanged old/source-support files against the original base, and the later three-payload independent full-source review. `goal-ninth-media-browser-root-preflight-v1.json` records current composition checks. The exact nine-path source diff is 139,233 bytes, SHA-256 `febd968afee6ceac380c4fc30d608e630bc64417fb6e1dc7d215a5dc2f8263c0`; `goal-ninth-media-browser-root-staged-proof-v1.json` records the byte-identical application, all 661 old tests preserved and all 39 new method identifiers. Strict lint for all seven changed Swift files and architecture contracts pass without formatting changes. README and the media documentation distinguish this source checkpoint from executed support.

This composition has not yet compiled or run. The next serial validation must execute all 39 new methods and the relevant existing decoder, service, file-browser and Button regressions; the earlier successful Button170 run is historical prerequisite evidence, not a current complete template result. Actual retained imagery, native drop/accessibility behavior, lifecycle teardown and performance still require their own evidence. All nine original goal gates remain open and unchanged, and no batch push has occurred.


### Ninth integration: retire accepted contribution-free physical attachments (2026-09-02)

The source correction is integrated over `1d5a20549984bfc19a668150d86e79c01ab4db86`. An accepted List row's complete physical receipt includes chrome that has no logical contribution. Previously, such a node could depart without removing its original attachment from that receipt. Attached survivors kept the receipt active, but the stale original witness could reject a later full physical handoff. The preceding a3df diagnostic observed the optional shrink succeed and later body value 42 fail to replace attached text 41; it did not identify the first executed rejecting guard. This correction repairs the source-proven missing cleanup, without promoting that causal hypothesis into an observed runtime result.

Each first accepted physical attachment now registers a node-local retirement association holding a weak physical receipt and the same original attachment witness. The witness retains only weak view, parent and runtime endpoints. Duplicate activation cannot substitute a fresh witness after identity invalidation. Actual attachment revocation consumes only the original target/attachment associations before token rotation and outgoing callbacks, and the receipt removes only a still-present identical witness. An owed departure can retire its stale original witness; a failed identity or attachment query alone cannot. No construction, logical declaration, task, descriptor or successor authority is granted by the association.

Contribution cleanup retains its existing order and results. Rollback and explicit receipt revocation withdraw only their own associations; explicit revocation publishes its irreversible phase before that withdrawal and preserves the original raw attachment array. Prepared handoff snapshots and all complete physical guards are unchanged. Expired weak receipt links are pruned during registration, never live original witnesses merely because their proofs are stale. The storage is local, proportional to accepted physical associations plus expired links since the most recent registration. No global traversal, provider callback, factory, runtime hook or new user payload is introduced.

The exact source patch adds 52 lines to `RetainedLazyListActivity.swift` and a new 437-line test file with three async methods. The first reproduces optional chrome removal and a following state update through a real List with the original two-frame flush, while retaining the departed chrome to distinguish actual cleanup from weak expiry. The other two preserve rejection after identity invalidation without departure, reject duplicate-witness refresh, and prove that replaying old cleanup cannot retire a reattached successor, another receipt, or a rollback/reaccepted association. All 664 existing test files remain byte-identical. The four separate inactive builder declaration-handoff failures are outside this patch and remain open.

Root read the full production diff, all three tests and the independent source report. The sealed two-path patch is 27,957 bytes, SHA-256 `9af8a8f0d7b4285f7d4ea5e3613aee104fdd61912f64f09a307f0da8712bc942`; `goal-ninth-physical-departure-root-staged-proof-v1.json` records its exact application, staged tree `fcf62bd8d6c9c5de1c3f9a15c9d98df3a839d841`, and preservation of all old tests. Strict lint and architecture checks pass with no formatting changes. Compilation and runtime execution remain pending. The next serial run must retain the original metadata and shrink diagnostic failures, run these three new tests, and include adjacent original physical handoff and contribution cleanup controls. There is no current pass, full-suite result, native parity or resource/performance qualification implied by this source checkpoint.

The README catalog count is corrected from 150 to 151 after a source inventory: 105 explicit base registrations, three default file-browser sample registrations, sixteen interaction registrations and 27 fixed light variants. Nested animation shape labels are not extra registrations. The trim example is a base registration without a light twin; the media template does not add a catalog entry. The 85 reviewed baseline images and comparison roster are unchanged. This is source registration accounting, not execution of every example or acceptance of an additional baseline.

All original goal text, prior outcomes and nine open acceptance gates remain preserved. The new documentation records the cleanup mechanism and its pending validation without reducing the original completion requirements. No batch push has occurred.


### Ninth integration: preserve the last published anchor coordinate basis (2026-09-02)

The reviewed anchor correction is integrated over `ed159760e48bd043a0aa41abec91e3393a2efcd6`. The prior a3df Public successor test observed 134 instead of its required 135. Its original measured prefix before row 3 was 92, so scroll offset 95 represented offset 3 within that row. A replacement installed fresh estimates with prefix 93 before its first factory canceled the operation. No replacement row table had been accepted, but the successor inherited those unpublished estimates and recaptured within-row offset 2. This was a lost accepted coordinate basis; changing the expected 135 or capturing the expected anchor after that loss would hide the defect.

The adapter now stages only the prior native scalar index, ordered tokens, token positions, spacing and exact attempt identity before installing a new estimate index. The two indexes have distinct array storage. No row, ViewNode, source, request, descriptor, lease, admission or authored payload enters this backup, and no extra index copy is added to a plan or candidate. Unpublished non-ready preparation restores the same stage after existing Button cleanup, even when a Candidate was never created. Ready-candidate disposal restores after its existing insertion, Button and row cleanup. Explicit revocation restores before invalidating the original authority, so existing inheritance moves the last published coordinate basis.

Restoration consumes only an identical still-pending stage. An obsolete destructor cannot overwrite a newer pending stage, completed table or transferred geometry. It leaves generation and accepted geometry authority invalid and unresolved work visible; it neither constructs rows, writes a scroll offset nor revives permission from old measurements. Actual native row-table publication clears the pending stage before callback-bearing payload release. A successfully accepted partial table therefore commits its fresh estimates, even when work remains unresolved or a later cleanup callback makes complete return false. This distinction preserves accepted bounded work without treating canceled unpublished estimates as authoritative geometry.

Seven new async raw-adapter methods cover cancellation before a Candidate exists, explicit factory revocation, ready-candidate discard and reorder, obsolete cleanup alongside a newer pending stage, accepted partial publication, revocation during publication cleanup, and a newer publication during old disposal. These are planning/publication controls, not native-window qualification. The existing public successor and one-round retained fixtures remain necessary execution checks of Runtime integration.

Five methods in two existing test files receive explicitly reviewed fixture corrections. Three layout-only queries now require their own current settlement receipt and current accessibility prepaint instead of hasPendingLayout, which also includes paint dirtiness intentionally retained for the next presentation. No extra query is added. The weak-proof test releases the complete closed fixture and runtime, whose last prepaint independently owns physical nodes, while retaining both original proofs and the empty adapter; it also requires runtime deallocation. The one-element/one-round test captures the original logical anchor before replacement and requires an actually accepted partial table. Its legitimate fresh-estimate coordinate is 96 rather than 95, while exact 1/1 consumption, factory [0], old row-2 height and unresolved-work checks remain. The Public successor keeps its required originalOffset + 40 (135), all seventeen historical assertions, and the existing separate ordinary query. New checks compare the original pre-reload anchor and actual row-3 viewport position before and after that successor-owned query.

Root read the entire production/test patch, explicit assertion inventory and independent source review. All 665 pre-existing test paths remain, with 663 byte-identical; only the two declared fixture files change, and every old test method name remains. The exact four-path source patch is 30,274 bytes, SHA-256 `ae3570470edaa521ece0d971a36c95fbde810de8abb1142c53b3a27b499bd453`. `goal-ninth-anchor-scalar-root-staged-proof-v1.json` records exact application at staged tree `308fd800c595dd0ea38b9dbab1d2cb224ffa51ec`, the two allowed existing-test modifications and all seven new identifiers. Strict lint and architecture checks pass without formatting changes. This composed correction has not yet compiled or run; the prior a3df 182-pass/9-fail UIA result remains historical failed evidence, not a pass rewritten by fixture edits.

The next validation retains all original UIA and other selected identifiers, unchanged default 128-element/four-round limits and explicit 1x1 negative controls. The seven new anchor methods must use a separate complete-class selection because adding them to the existing UIA191 selection would exceed its unchanged identifier bound. The warm 50,000-row extent problem, independent inactive builder handoffs and native smoke failures remain unresolved at this source checkpoint. All nine original completion gates and their acceptance requirements remain open; no batch push has occurred.


### Ninth integration: first composed build failure and scoped media imports (2026-09-02)

The first fresh Shape111 selection at frozen `5e246fc46b7c37ff13a0f617b4e83833ff91b579`, tree `c18ddcc8114b402257f5130567d1750567696e9e`, failed during compilation before any XCTest case started. The compiler reported one unique source diagnostic, emitted twice: `DemoMediaBrowserTemplate.swift:225:83` used a CGFloat type name made ambiguous by importing both all of Foundation and the public WinSwiftUI facade. The test filter was not reached; all 111 selected methods are unrun, not failed test cases or partial passes. Other prepared selections, including the 39 new media methods, were not launched at this head.

The direct child PID 52640 and runner both closed naturally with exit 1 after 76.328 seconds, without timeout or termination. `shape111-5e246fc-c1af6f2ea1984a4e83daba82e944e016/raw.log` contains 11,801 bytes, SHA-256 `ac92eb6c7949f370620075f67ace3df4a8f1923ed05c674590b6267de5d346d1`. `goal-ninth-shape111-5e246fc-compile-failure-v1.json` independently compares source/index endpoints, source selection pins, the exact diagnostic and absence of started tests. A separate post-closure CIM observation found no matching compiler, formatter or native processes; that is a point observation, not continuous descendant attestation. No fresh compiler-generated Builder discovery was accepted from this failed build.

The template now imports only Foundation.NSLock, Foundation.URL and Foundation.UUID, matching the scoped-import convention already used by the shared dashboard. Its CGFloat parameter, thumbnail heights, layout, decoding, ownership and interaction behavior are otherwise byte-identical. No platform-only view API, conditional numeric workaround or test expectation changes. The one-file diff is 592 bytes, SHA-256 `72016a80993e91754dfa4fee5d75b6cb6ede55cba8d1eaa5a13aa3f690b8eac5`; `goal-ninth-media-scoped-imports-staged-proof-v1.json` records exact application and all 666 existing test files unchanged. Strict lint and architecture checks pass. This import correction still requires a fresh successful build; a source fix is not a retroactive pass for the failed run.

The same complete-class test selections will be rebound to the next clean source commit, retaining all original method identifiers and work/time limits. The expanded media regression selection also includes the existing real decoder, image service, shared URL-validation service and file-browser model/render preparation classes. The separate historical File14 interaction timeout remains fourteen unknown outcomes and is not replaced by these source inventories. All nine original goal gates remain open; their requirements and the earlier evidence are unchanged, and no batch push has occurred.


### Ninth integration: composed regression compilation declarations (2026-09-02)

The second Shape111 build at frozen `20333f939d5c878a3a02cf0faf24ca25e7d02870`, tree `e8877f6afd5913d32079b4df209b371fc6242856`, passed the media template import issue and reached the test target. It still failed before any test started. Two unique source diagnostics account for 36 repeated compiler lines: `TransformZeroColumnTests.swift:85:37` used an ambiguous identity shorthand, and `LazyListAnchorScalarStageTests.swift:263:36` could not find Rect without an explicit import of its defining core module. This is not execution of any selected method; all 111 cases remain unrun at that commit.

The direct child PID 71272 and runner closed naturally with exit 1 after 97.047 seconds, with no timeout or termination. The retained raw log in `shape111-20333f9-4f04d070a5e14813b566d71d69ecfac6` contains 3,731,043 bytes, SHA-256 `d3c935e3e6601466ce3bc79294886d248f4aaad4b64e8d0bb683c78d7214503e`. `goal-ninth-shape111-20333f9-compile-failure-v1.json` independently verifies source/index endpoints, all selection source pins, those exact diagnostics and zero started tests. The following CIM observation found no matching compiler, formatter or native process. Existing compiler warnings were left unchanged.

The new anchor test file now imports SwiftWindowsCore, and the collapsed-axis test spells its intended argument as `Transform2D.identity`. Every test name, expected coordinate, budget, action and assertion remains; the second change resolves the intended overload rather than changing the composition being tested. Production code does not change. `goal-ninth-regression-geometry-names-staged-proof-v1.json` verifies the exact two-file, 1,304-byte patch, SHA-256 `95a1944c323ad0edb44577c579d57f125bd575702785cc76b6436a058612e5eb`. All 666 pre-existing test paths remain and the other 664 files are byte-identical. Strict lint and architecture checks pass without formatter changes.

The next run still requires successful compilation and actual execution of the original complete-class selections. No Builder compiler roster, native launch, visual qualification, full-suite pass or completed goal gate is inferred from this failed build or its source correction. All nine original gates and prior evidence remain unchanged; no batch push has occurred.


### 2026-09-02 current-journal completion peers and frozen ba375b8 regression evidence

This entry adds implementation and verification detail without changing the original scope, pinned SDK, or any of the nine completion gates. All nine gates remain open. The source immediately before this repair is `ba375b803c784484379ab101549e389b4d3bd99a`, tree `44c57d17ce34554d7f1dc906c105ee2e57b58890`.

A completed contribution can share its actual attachment with another accepted output in the same adoption journal. `recordCompletedNode` previously treated the first output as absent when the second completed, because only the second receipt appeared in that individual incoming list. The repair excludes a previous receipt from inferred completion absence only when this exact journal still stores that exact receipt under its exact group identity. Sharing a target, attachment, or physical lifetime alone grants no exemption. Explicit absence, property replacement, accepted departure, physical retirement, activation and revocation retain their existing behavior. The production change is three lines, including two comment lines.

Four new `RetainedLazyListCompletionPeerTests` cover opposite completion order and independent peer revocation, replacement by a later journal on a shared physical attachment, explicit absence followed by physical departure and reattachment without revival, and an actual property replacement that still retires its peer. Root preserved all 666 pre-existing test entries. The complete staged source patch is `artifacts/goal-ninth-completion-peers-root-formatted-v1.patch` (17,134 bytes; SHA256 `3a21dc6ec89bfec02372cd1c126e25cbc40170b3e01d6788256d55cf908cea69`). Its source proof is `artifacts/goal-ninth-completion-peers-root-staged-proof-v1.json`. Strict lint first identified two new-test line breaks; the only change to the frozen private test was those two line breaks. Corrected strict lint and contracts passed. This is source integration evidence; the four new cases have not yet run at this entry.

Fresh, serialized runs on the preceding ba375b8 source completed 111/111 shape cases, 116/116 native unit cases and 7/7 anchor scalar-stage cases. The physical lifetime group completed 29 cases with 28 passes and one failure: the unchanged same-target contribution test rejected the first peer at its receipt-active assertion, motivating the narrowly scoped correction above. These are focused XCTest results, not actual native-window, GPU, visual, hardware-performance, full-suite or release qualification. Exact run endpoints, expected method rosters and outcomes are retained in the corresponding `goal-ninth-*-ba375b8-reconciled-v1.json` artifacts. Each naturally closed run was followed by a separate empty process census; the census is a point-in-time observation, not continuous descendant attestation.

The unchanged 79-case builder group also completed naturally, with 73 passes and six failures (`artifacts/goal-ninth-builder79-ba375b8-reconciled-v1.json`). The two shrink cases now retain the same tail node, present 42 after the second write, and reject a write through the retired optional binding. Their only remaining assertion failure is four row factory calls versus five recorded root metadata evaluations. The fresh 35-event diagnostic records two initial root evaluations with one row construction, followed by one construction for each accepted update; the retired write triggers none. This distinction is recorded in `artifacts/goal-ninth-builder79-ba375b8-shrink-diagnostic-v1.json`; neither case is declared passed, and the remaining count oracle still requires source review. The other four inactive declaration failures remain open. No original assertion was changed in this repair.

The separate Media152 run exceeded the unchanged 900-second bound with no observable case starts or terminals. All 152 case outcomes remain UNKNOWN. Its direct runner closed, then root closed only the remaining owned XCTest process after checking its creation identity and executable through a retained process handle. The separately saved post-cleanup census was empty. The raw log and original timeout record remain intact. Two bounded non-suspending samples provide attachment-validation construction context, but do not establish an exact causal call stack or a completed case. Source inspection found repeated full attachment validation during descriptor-owner authorization; no production exponential recursion or sufficient explanation of the timeout has been established. A retry is not qualification, and no timeout, test, lifecycle guard, or completion threshold was relaxed.


### 2026-09-02 preserve owned logical list geometry above the paint-coordinate limit

This source change addresses the already recorded warm 50,000-row public realization failures; it does not change their assertions, the 128-element/four-round default budget, the one-element/one-round negative cases, or any original completion gate. It follows the completion-peer repair at `bc34e4244df14768e12f0f120d1171b4a8a1f15b`. All nine completion gates remain open, and execution of this new geometry source is still pending at this entry.

Logical list range and prefix coordinates must retain finite Double precision even when they exceed the scene's one-million-point coordinate limit. `Runtime.swift` now preserves the actual native list content height, actual native row prefix Y, recognized sole-child logical carrier height/Y, and the owning vertical scroll offset. Ordinary authored row heights, horizontal coordinates and widths, unrelated scrollers, and native rows containing a separate nested adapter retain their existing limits. The recognition walk uses the actual nonhidden child chain, original parent/runtime identity and the existing depth bound; it stops at nested scroll owners and rejects invalid topology. It introduces no persistent authority marker or global state.

Raw logical stack padding, spacing, and sums are validated before a cache hit can conceal invalid input. The shared traversal context checks the actual sole child's assigned finite Y, height and maximum Y after placement and after late position changes. On rejection, pending/cache keys that could conceal the invalid geometry are cleared. The guard evaluates actual allocation rather than inventing an estimate for a child; a small finite allocation may therefore remain valid even when an unallocated estimate plus padding would overflow. Nonfinite/overflowing extents and malformed logical layout cannot be certified merely because ordinary scene sanitation would clamp them. Repeated clean queries must still reject the malformed input until an explicit finite repair.

Twenty-three new `LazyListLogicalVerticalGeometryTests` cover the already-warm public 50,000-row target, deep index 40,000 with prefix 1,280,000, measured settlement and prepaint in the same request, bounded emitted paint coordinates, hit testing and UIA invocation, scroll observation, actual sole-child wrappers, nested/unrelated scrollers, multi-child rejection, nonfinite and overflowing input, cache reuse and late position overflow, depth exhaustion, and an actual finite-allocation counterexample. The original 191-case accessibility group and all 667 pre-existing test entries are unchanged. The renderer still subtracts the Double logical offset before bounded scene output; no GPU, scene, query predicate, or allowance was changed.

Root applied the independently reviewed two-file source packet to an exact matching Runtime preimage. The only additional production edits were required formatter line wrapping and removal of redundant guard parentheses; an unrelated formatter import move was restored. New-test formatting also made four closure argument labels explicit and replaced an equivalent cleanup forEach with a for-in loop. All other new-test tokens and quoted strings match the reviewed packet, and all existing tests remain byte-identical. Strict lint and architecture contracts passed. The complete 60,120-byte source patch is `artifacts/goal-ninth-logical-geometry-root-formatted-v1.patch`, SHA256 `ecc1067645466decfa6678b9e51da0445adf7f9f5a57fa4e17ec82b6c002d008`; the full staged proof is `artifacts/goal-ninth-logical-geometry-root-staged-proof-v1.json`.

The required next evidence is fresh execution of the 23 added cases, the original 191 accessibility cases, and the physical/contribution regressions on the committed source. Source review and compilation alone will not close those cases. General malformed-layout recovery, hardware GPU fidelity, real native accessibility, macOS comparison, performance/resource gates, full-suite validation and release qualification remain separate open work.


### 2026-09-02 correct new geometry test API calls after the first compile

The first Warm23 attempt on `ed114302316f5894d8c9401401d26cac66f93c13` stopped during test compilation, before any XCTest case started. Root preserved the failed run and source/index endpoints in `artifacts/goal-ninth-warm23-ed11430-compile-failure-v1.json`: natural child/runner exit 1, 140.234 seconds, nine distinct diagnostics repeated across 13 compiler batches, raw log 2,033,221 bytes with SHA256 `fdc8865e5615bb443b72c8ae0b2612873933bab5065788a99e9aa09818100ce0`. A separate post-closure census was empty. This is zero executed cases, not 23 test failures or passes.

Only the newly added `LazyListLogicalVerticalGeometryTests.swift` changes here. Its two visibility assertions now explicitly unwrap the existing optional `isOffscreen` API before requiring false, so absence still fails. Seven inset constructions now supply the existing initializer's required zero-valued remaining edges; their nonfinite, overflow and negative input values are unchanged. All 23 method names, original comparison bounds, and production code remain unchanged, as do the other 667 test entries. Strict lint and contracts pass. The complete source diff and allowed-single-test proof are retained as `artifacts/goal-ninth-logical-geometry-test-api-root-v1.patch` and `artifacts/goal-ninth-logical-geometry-test-api-staged-proof-v1.json`.

The original criteria and nine open completion gates are unchanged. Warm23, Physical33 and the original UIA191 still require fresh execution on a successor source revision; the earlier failed compile supplies no execution qualification.


### 2026-09-02 observe the existing native foreground activation result once

The prior actual native smoke run remains a 24-of-27 result with three failed predicates. This change adds a diagnostic for one unresolved source hypothesis; it does not make that run pass, alter its workload, or infer that foreground activation caused the observed idle messages.

`Win32NativeWindow` records the Boolean returned by its existing `SetForegroundWindow` call immediately after that call, using the existing optional smoke observer and the actual window key and surface generation/sequence when available. The original call order, return value, ShowWindow/UpdateWindow behavior, input handling and message pump are unchanged. An ordinary application without the observer evaluates none of the optional diagnostic arguments. The observer reserves one recording attempt per fixture before invoking the common recorder, so rejected recording cannot be retried into later evidence. The new event is appended as raw value 85; all original 0...84 values remain exact. A true value records only that call's success; it is not proof of continued foreground ownership or idle. A false value is not proof of the cause of a later native message.

Eight new `Win32NativeSmokeForegroundActivationTests` cover old vocabulary preservation and the sole appended kind, true and false wire values and missing surface metadata, no-recording and optional-observer behavior, one attempt across windows and separate fixtures, sticky invalid-input rejection, and acceptance of both Boolean values without changing the existing 27 predicates. Root applied the complete four-file source packet byte-for-byte, including the `docs/NativeOwnedSmoke.md` explanation, and preserved all 668 existing test entries. Strict lint on the three Swift files and architecture contracts passed. The exact 18,036-byte source patch has SHA256 `96a79c9f68b0776f579c37336068856d8770c1f94d0454ed99d28d4e7e09fed5`; `artifacts/goal-ninth-native-activation-root-staged-proof-v1.json` records the full comparison. The new tests have not yet run.

The matching artifact controller is a separately preserved successor, `artifacts/goal-ninth-native-smoke-controller-activation-v1`. Root independently verified that its only code change is the single kind-range byte 84 to 85. Its output schema only appends the new event and updates the exact observation source byte/hash pin. Its binding creator changes only that controller directory and the two controller/schema hash constants. All original predicate names, provenance fields, 4096-record/one-MiB trace limits, 64-probe workload, query ordinal 31, N16/A32 caps, native fixture/watchdog and controller cleanup time bounds remain unchanged. The source observation pin is 16,530 bytes, SHA256 `6f084c5d57f84e8b173b2352d94519468ee3c35128b3d2e9fef45f5648287527`.

No controller or binding creator was imported or executed during this source/artifact preparation. No binding or new native run was created. A future native diagnostic attempt still requires fresh passing Native116 and new activation tests, a current committed source/PE association, and root review. The old failures, Media152 timeout, remaining builder work, full validation requirements, and all nine original completion gates remain open.


### 2026-09-02 account for deferred List setup separately from subsequent row work

Source review of the two remaining shrink count failures established that their absolute metadata/factory equality compared different initial construction scopes. The host constructor calls `componentHost.setComponents`, which reloads and collects root metadata before layout. `windowDidCreate` activates the presenter, sets its real display size/scale and reloads root metadata again before the first rendered frame. The data-backed List stores its row projection during both metadata collections; only selected materialization executes the row factory. This explains the observed initial metadata count two and factory count one in the unchanged ba375b8 diagnostic. Eager ForEach uses a different construction path. Adding a needless initial row construction would conflict with deferred list behavior.

Only the two reviewed methods in `CanonicalViewBuilderMetadataTests.swift` and `DeferredListProjectionShrinkDiagnosticTests.swift` change. The deferred List route explicitly requires exactly one initial factory after the fixture's settled creation, records both actual initial counters, and requires exact equality of subsequent factory and metadata deltas. The two eager routes retain their original literal absolute equality. Both methods now additionally require zero new metadata evaluations, factories and custom-body calls after the retired optional binding write; existing checks that the binding stays at three, no optional node revives, the tail stays at 42, and identity/tags survive are all retained. Every per-metadata-sample no-custom-body check remains. No tolerance, extra frame, new lookup, factory invocation, callback, or production change was added.
...
### 2026-09-02: preserve original List Invoke completion and record dashboard validation

This entry adds implementation and evidence detail without changing the original product scope, acceptance thresholds, or any of the nine unchecked completion gates. The changes below are not a release or full-suite qualification.

The reviewed List Invoke repair is integrated from the independent source packet ending at `5db0b5064dca108eedf2da037869538e0d0a67f6`. One outer lazy-list resolution budget now spans the original preflight, action, nested requests, and any permitted completion query. The default 128-element/four-round allowance is unchanged. Before dispatch, the implementation captures the original logical container, token, item and weak physical action target. Its separate non-inlined dispatch frame releases the temporary strong target before subsequent currentness checks. An unchanged ordinary row needs no extra preparation. A successful action that leaves its accepted adapter unprepared may perform at most one completion query, only with remaining original allowance, no queued layout callbacks, and no Realize request. Both initial preparation and every continued construction check remain qualified by the original accessibility target. Completion requires the same physical node, fresh binding in the original container, and current original logical item. It does not authorize a replacement target, reveal or scroll the row, refund work, drain queued callbacks, or retry the action.

The single changed assertion site in `PublicLazyListAccessibilityTests.testRealizeAdoptsAndLaysOutTheActualActionTarget` corrects an older inconsistent oracle: Realize must leave a current layout-settlement receipt while paint remains pending and dirty. The former assertion required the general pending-layout flag to be false even though query settlement did not consume pending render work. The source-history audit traced that assertion to `41c9e82f9ed1dfa286ec51c9ef80c962f1664c8d`; the pending-work and settlement contracts predate it. The change keeps the actual bounds, offscreen state, Invoke, action and immediate-focus assertions. All other 672 previously tracked test paths remain unchanged. It neither adds a repair query to the fixture nor weakens its behavioral checks.

The new `LazyListUIAInvokeCompletionTests` contains eleven methods with fourteen configurations. Its positive path calls real native Invoke and immediately requests focus before any explicit repair query, checking the original node, stable identity, bounds and actual row factories. Negative cases cover the one-element/one-round allowance, queued callbacks, logical replacement and physical attachment ABA, adapter transfer, scrolling ABA, host stop, mutation termination and nested attempts to recharge the budget. These methods are UNRUN at this integration point. There is no dedicated new destructor/weak-expiry test, and the existing action-own preflight boundary is not expanded. The separate native bridge that discards an ordinary Invoke's Boolean result remains an open transport defect; this source repair does not claim to fix it or the provider's asynchronous-Invoke compliance.

Root verification before commit: contracts and strict lint passed for all four staged Swift files (`8a3aa6` / `f1d22c`). The two formatter changes preserve all non-whitespace bytes and quoted spellings; their full diff was read. `goal-ninth-uia-invoke-root-staged-proof-v1.json` records the exact four-path patch, all prior test-file preservation except the explicitly reviewed Public site, and the eleven new method identities. The formatted source patch is 43,966 bytes, SHA-256 `a04afb356aba5ed79e7692a26c774c676e4787da0a16d93bcec221b0f9d2c630`; its staged source tree is `d4fedc93a03898f26437ddb914ff82671d738906`. These are source and formatting checks, not compiler or runtime evidence. The next validation obligation is the unchanged fifteen Public methods plus the eleven new methods, followed by the wider UIA regression cohort.

Dashboard evidence belongs to exact prior commit `3787ed1e6050a1676403ed5fad31661a7639f938`, not this new source tree. `DemoDashboardDataModelTests` reported 25 started and 25 passed, with no failures or skips, in a 306.688-second build-and-test attempt. The test class itself reported 0.018 seconds. The reconciled raw log is 34,775 bytes, SHA-256 `ad9922a4e27d6692492985c414ea009610cf68783d23bfe6ba8190b203c7344d`. Source/index endpoints matched, the direct child closed naturally, and a separate census found no remaining matching process. This verifies the selected model tests, not retained interaction or visible output.

The separate `DemoDashboardDataInteractionTests` attempt reached its unchanged 900-second deadline after 900.031 seconds, with zero emitted case starts or terminal case rows. All eleven selected cases remain UNKNOWN, not passed, failed or skipped. Its raw log is 231 bytes, SHA-256 `0ec4084a50e32821cfef7b2a42524d4114eb94476e62aa3a4f5da6dcd21e23db`. The original cleanup-required flag remains preserved. A separate closure procedure verified creation times and executable paths through retained handles for all six owned identities, terminated only the remaining XCTest child with exit 143, observed the other five close without further termination, and completed a clear process census. `goal-ninth-dashboard-ui11-3787ed1-timeout-reconciled-v1.json` ties those observations to unchanged source/index endpoints.

One noninvasive, nonsuspending sample and offline symbolization found Button witness validation and child-matching addresses among 42 stack-word candidates. The actual sampled instruction was `swift_retain`; those words are not a reconstructed causal stack, a case identity, or a measurement of total work. Repeated Button validation is an investigation candidate, not an established cause or repaired timeout. The earlier Media152 and File14 timeouts are still unknown. No timeout was extended, no UI workload retried, and no desktop capture, retained snapshot, gallery comparison, hardware motion evidence or original completion gate is claimed here.


### 2026-09-02: distinguish primary native launch from requested-window activation

The original scope and all nine completion gates remain unchanged. This is a narrow startup behavior repair, not qualification of native idle, fairness, timing, or release readiness.

`WindowCoordinator` now carries an internal startup intent derived only from the existing `isPrimary` value. Primary launch awaits the existing native show operation after preparation and before presentation completion. Fresh explicitly requested windows, including Settings and value windows, still await activation; existing-window reuse retains its prior behavior. Cancellation does not invent completion of an outstanding show reply. Operation errors retain their propagation and awaited rollback ordering. The smoke-session hook forwards the intent instead of silently discarding it.

The existing native show sequence is extracted into a synchronous, nonescaping helper used by the actual window operation: enable input, call ShowWindow with SW_SHOW, call UpdateWindow, require successful InvalidateRect, then sample geometry. The existing ignored ShowWindow/UpdateWindow return values remain ignored, while invalidation and geometry errors retain their original result. The case returns directly after that helper, so geometry is not sampled twice by the common epilogue. Activation and the common epilogue are unchanged. ShowWindow may itself activate a window; this change is not a promise of a nonactivating launch. It removes the explicit foreground-activation operation from primary launch while retaining the existing show operation's checked invalidation and geometry behavior.

The independent packet was based on `d8fe4c178c2284dd37adf03dd6f1a7c59dcf0f53`. Root verified that all five old source/test preimages still matched at `8e48637d8b9faf4855e1938e88732234a1e4783a`, applied exactly six reviewed paths, and verified every postimage against that packet. The one old test-file edit is only the three-byte `, _` closure parameter adapting its fake hook; its body, assertions and behavior are unchanged. The other 673 previously tracked test paths remain exact. No old native smoke predicate, allowance, trace limit, workload or oracle is changed.

The new `NativeWindowStartupPresentationTests` contains twenty async methods: eleven intent, ordering, reply, cancellation and failure cases; six cases using the actual native show's extracted operation helper; and three real headless coordinator cases for primary/requested/reused windows and rollback. They are UNCOMPILED and UNRUN at this commit. They cannot replace an actual Win32 startup and smoke run.

Contracts and strict lint passed for the six staged Swift files (`f78d44`, `455dd8`). The whole staged source patch is 31,192 bytes, SHA-256 `22db59af5dbffdea2768d94d0b64f79271f46293577ed4f85baf24d1a6ff81fc`, exactly the reviewed packet; staged source tree `16ccbf87fa07619813df63086271f5f7f78b8aa2`. `goal-ninth-native-primary-startup-root-staged-proof-v1.json` records the six paths, twenty new method identities, and preservation of old tests. No formatter changes were necessary.

The last actual native smoke result remains the separately recorded 24/27 failure at `1aacbb2`: actor progress between backlogged turns, the required 32-record turn/continuation exercise, and three seconds of unforced settled idle were not all satisfied. Its failed foreground request and later WM_SYSTIMER traffic are observations, not a proven causal connection. This startup change does not repair or waive those predicates. Fresh serial unit/coordinator tests and a freshly bound actual native run are still required.


### 2026-09-02: identify exact native scroll and animation registration writes

This additive foundation preserves the original scope and all nine open gates. It does not complete ordinary declaration attachment, controller termination, or the previously held partial-arrival repair.

New internal recording routes identify an actual scroll insertion or animation publication with a one-shot reference receipt. They require the node's already installed runtime to match the original runtime before any allocation or native write; the scroll route also requires that runtime's current registry. Existing scroll membership returns an explicitly non-owning result. Normal routes keep their existing order and idempotence, allocate no receipts, and remain the only production callers. The receipt contains identity keys, terminal state, and weak origin references, not a node, controller, observer storage, history, callback, or animation payload.

Removal consumes the original receipt before checking its origin and exact watched entry. A wrong-runtime attempt therefore spends the same permission seen by all aliases. Normal replacement, removal, detach, registry replacement, sweeping and compaction revoke the appropriate watched identities; a surviving recorded entry keeps its identity through compaction. Publishing observer storage or animation state revokes the prior cleanup permission before releasing an old payload, including equal assignments, same-storage assignments, nonempty replacement and same-key mutation. That invalidation does not unregister the node, reset history, cancel replacement motion or clear handlers. A spent wrong-origin watch can remain as payload-free bookkeeping until a later native change or owner destruction; it cannot regain authority.

Both new recording APIs are intentionally unused by higher-level production attachment code at this stage. In particular, the existing ordinary animation registration precedes installation of node.runtime; replacing that call with this API would reject it. A later consumer must resolve that ordering and preserve original physical attachment and terminal cleanup handles. No generic cleanup ownership, controller-close guarantee, or fix for the held F4 off-table arrival is inferred from this foundation.

The reviewed source packet ends at `e210bf292e905ffa70475e97b63cdd9820e08c5b`, based on `d8fe4c178c2284dd37adf03dd6f1a7c59dcf0f53`. Root independently applied the original patch and intervening root changes in both orders using a private index; both produced source tree `b7c88d4eceab125ea5ccaf92c13682aaa1f9e6ae`. The original inverse reconstructed the complete current parent tree. This verifies composition with the newly committed Invoke and native startup changes rather than assuming the older Runtime.swift still matches. The resulting three-path patch is 48,388 bytes, SHA-256 `d4bfbc6437c48da7157d9246c4a3c429ced3eefa59e90f435695b17f926641f0`.

Contracts and strict lint passed (`dcbf0f`, `9be71f` / `345d8d`). `goal-ninth-native-registration-root-staged-proof-v1.json` verifies that all 675 prior test paths remain unchanged and records 29 new async methods in `RetainedNativeRegistrationTests`. They cover actual versus non-owning registrations, one-shot and wrong-origin cleanup, replacement and ABA, callback release, compaction, weak lifetimes, animation publication/sweep, and detach/restart. These tests remain UNCOMPILED and UNRUN here; no performance measurement or wider runtime pass is claimed. The independent review also preserves the limited fixture meaning: its empty controller terminal methods are not controller-cleanup evidence.


### 2026-09-02: remove Button validation key-path overhead without reducing checks

The original acceptance criteria and all nine open gates are unchanged. The dashboard timeout remains unresolved pending actual execution evidence.

`RetainedButtonActionAdoption.isCurrent` now uses an explicit loop over the same `witnesses.values` iterator instead of `allSatisfy` through a key path. It retains the original operation check before the first witness, the same order, a fresh unchanged `matchesCurrent()` call for every visited witness, and immediate return on the first refusal. The complete Witness class, operation predicate, guarded writer methods and completion methods remain byte-identical. No owner read is hoisted, no attachment or identity proof is refreshed, and no permission is cached. A plain query refusal remains reversible if the original state is restored; a failed authorized write still permanently rejects its operation.

Optional diagnostics, disabled by every existing call site's default argument, record only check count, loop witness visits, fixed roster size and a sticky overflow flag. Counters saturate at UInt64.max instead of wrapping. A copied snapshot contains no executable payload or ownership reference and grants no authority. The counters deliberately exclude direct witness checks made inside other writer methods; they do not measure complete reconciliation or rendering cost. No host routing, global observer, per-check array or log was added.

The cost model remains linear in the captured witness roster, its child tables and runtime references for each successful check. The source audit identifies repeated complete checks during child matching, but this change only removes a constant-factor source of overhead; it does not reduce the number of checks or establish an asymptotic improvement. The actual Dashboard sample was not a causal unwind and does not prove this is its dominant cost.

The private successor `f2b372929fab2b35deb9c82110530937a6920339` fixes two access-control mistakes in its new tests through the existing internal runtime accessor; no old test was changed. Root read the complete patch and independent review, verified the original production preimage against current HEAD, and verified both postimages. Contracts and strict lint passed (`d3560b`, `c468ab`); no formatting changes were needed. The exact two-path patch is 19,952 bytes, SHA-256 `3e721766ec883a8db3dd6217d207a3a727a4494655e43a635161386ccc178589`, staged source tree `71e9ca93c828f54bc1baab58c75519610f78288d`. All 676 prior test paths remain unchanged in `goal-ninth-button-validation-root-staged-proof-v1.json`.

Seventeen new async methods cover disabled diagnostics, repeated full visits, first-refusal stopping, operation refusal with no visits, existing weak lifetimes and ownership/identity/attachment checks, reversible query versus permanent write refusal, authorized table advancement, snapshot lifetime, and explicitly seeded arithmetic overflow. They are UNCOMPILED and UNRUN here. Their later pass would establish these selected behaviors and scalar counts, not elapsed-time improvement or repair of DashboardUI11, Media152, or File14. Those workloads retain their separate unresolved outcomes.


### 2026-09-02: clamp bounded standard-integer Steppers before overflow

The original product goal and all nine open completion gates remain unchanged. This correction implements one concrete control behavior; it does not expand the supported subset or qualify unrelated controls.

The generic bounded Stepper previously called advanced(by:) before clamping its result. For example, an enabled UInt8 value of 1 with bounds 0...10 and step 2 could underflow on Decrement before the lower-bound clamp ran. The bounded generic initializer now passes the raw step and a subtraction flag to a private helper. For the twelve exact standard integer types supplied by the installed Swift 6.3 SDK, the helper clamps the starting value, computes the remaining unsigned distance to the selected bound from bit patterns, and compares the stride magnitude before narrowing. A step reaching or exceeding that distance returns the bound. Otherwise the distance comparison proves a representable interior result before its bits are reconstructed. Neither a full signed span nor a wide unsigned value must fit Int or Double, and the minimum signed stride is never negated on this path.

Admission is restricted to exact Int/UInt and their 8-, 16-, 32-, 64- and 128-bit standard types. Every custom Strideable, including user-defined FixedWidthInteger conformers, retains its original raw delta, pre-clamp, authored advanced(by:) call and final clamp. The unbounded generic helper is unchanged. The separate concrete Int and Double initializers, all twenty public initializer signatures, callback ordering, binding reads/writes, named-direction availability, accessibility and input routing are unchanged. Generic negative and zero steps retain their prior policy; concrete Int still normalizes nonpositive steps to one. Previously overflowing unbounded or custom operations are not claimed repaired.

Root read the complete final private patch at `e7319e0f0b76809632e495ac1a548745718e11fa` and its independent arithmetic, SDK, preservation and fixture review. The complete current Views.swift preimage still matched its `3787ed1` base. The two postimages matched the reviewed packet before root style checks. Strict lint found one new-test call mixing closure arguments with a trailing label closure; the formatter made no change, so root changed only that call to use the existing explicit `label:` argument. A whole-file single-substitution proof preserves all assertion expressions, quoted spellings and eighteen methods. Production bytes were untouched by this correction. The final strict lint and contracts passed (`09dfd1`).

`goal-ninth-stepper-integer-bounds-root-staged-proof-v1.json` records two paths and all 677 old test paths unchanged. Its final source patch is 23,012 bytes, SHA-256 `310fa78c3d8a7d99a5c011dea56af9adf16f60b5d2fcb0cf54fa698b7de6bf56`, staged source tree `6437a72c06fbc38696b49b9f6330a9b13a4d2ef8`. The eighteen new async methods exercise actual public retained controls across all twelve integer widths, narrow and full signed spans, values above Int.max, minimum strides, endpoint clamping, original step policies, disabled controls, exact editing/write/invalidation order, keyboard and pointer activation, and floating/custom/unbounded fallbacks. They are UNCOMPILED and UNRUN here; the old four Stepper methods are preserved for wider regression validation. The wide minimum-stride fixture expectations target the current Windows x86_64 environment, not an untested 32-bit target.


### 2026-09-02: retain the first 110-case compile failure and correct its new-test API use

The fixed 110-case attempt at `b7480af55bcb94d3ab859c75f1db14f8f8c09206` failed during compilation after 201.578 seconds. No XCTest method started. All 110 selected methods remain UNRUN, not failed test cases or passes; this was not another 900-second timeout. The selection includes the fifteen original Public List accessibility methods and the eleven Invoke-completion, twenty native-startup, twenty-nine registration, seventeen Button-validation and eighteen bounded-Stepper methods.

`goal-ninth-repair110-b7480af-compile-failure-v1.json` independently reconciles unchanged complete source/index endpoints, natural direct-child exit 1, the exact selection, and the raw log: 3,706,305 bytes, SHA-256 `b6f249720d92e213478eed879ead1f0b80ad604ace2063644ceaab8f4747f590`. There are two unique compiler diagnostics, repeated 36 times, both caused by the new Stepper test's nonexistent `ViewNode.accessibilityRole` property and its consequently untyped `.button` expression. A separate point-in-time process census found no remaining Swift, XCTest or formatter process before correction.

The correction changes exactly one line in the new `WinSwiftUIStepperIntegerBoundsTests`: it now requires `decrement.accessibilityTraits == .isButton`. The real node API starts with empty traits and Controls.button adds `.isButton`; Stepper's default enabled fixture adds no other traits. The author independently confirmed that this preserves the specific Button semantic assertion without an additional accessibility query, layout pass, action, or weaker contains-only check. Label, focus, keyboard dispatch, editing/value/invalidation assertions and all eighteen method bodies outside that line are unchanged. Production code and the other 677 prior test paths remain exact.

Strict lint and contracts passed (`7abe19`). The one-path correction is 984 bytes, SHA-256 `024c36f7017dcaa3b0323112cebba5a6e3a103d3b0daf75fee1b5e521770a1ae`, staged source tree `f7ef3f31dd2e34643871520fc447f1a041a14820`; its whole-file substitution and staged preservation receipts are retained separately. The same 110 methods and unchanged 900-second allowance must be rerun on the corrected clean commit. The original goal, its thresholds, all nine open gates, and all previous unresolved runtime outcomes remain unchanged.


### 2026-09-02: preserve the incomplete 110-case runtime result

The corrected 110-case cohort at `3fe8af69ca10998d53d439010516ee7eef06424a` compiled, then ended naturally with direct-child and runner exit 1 after 502.079 seconds. Its build-complete marker reports 341.73 seconds. This was not a timeout. There were 74 distinct starts: 72 terminal passes, one terminal failure, and one started case without a terminal result. Another 36 methods never started. Neither the interrupted case nor an unstarted case is counted as a pass or an ordinary terminal test failure.

All fifteen PublicLazyListAccessibilityTests, twenty NativeWindowStartupPresentationTests and seventeen RetainedButtonActionValidationTests passed. LazyListUIAInvokeCompletionTests had ten passes and one failure. RetainedNativeRegistrationTests had ten passes, one started unknown and eighteen unstarted methods. All eighteen bounded-Stepper methods were compiled but unstarted. These are selected headless results at this exact checkpoint, not a full suite, actual native-window pass, performance measurement, or validation of later edits.

The failed Invoke method is `testActionDeletionAndAcceptedSameKeyReinsertionDoNotReviveTheOriginalProvider`. Its seven assertion diagnostics begin at the required accepted-absence check inside the original action, before post-action completion. Source investigation found that its synchronous root reload evaluates a direct List initializer under the action's old row build context; that context can reject the proposed projection before the absence is committed. Direct List roots are valid public scene content, and a corresponding production window-content factory boundary is under investigation. No deletion, token-identity, query-budget or no-post-work assertion has been removed, and an extra wrapper View is not being used to avoid the failing construction path.

The raw log also contains a fatal Swift exclusive-access conflict. The last started case is `testNilStorageReleaseCanPublishReplacementWithoutOldCleanupRemovingIt`, whose old observer payload synchronously publishes replacement storage during destruction. The observed stored-property setter holds its write access across that release. A separate production correction is being reviewed; the real reentrant callback and all existing assertions remain required. Combined stdout/stderr buffering places the fatal block before some XCTest text, so raw line order does not establish its wall-clock order or supply a missing terminal case result.

The immutable run directory is `repair110-3fe8af6-842d5c28d9024132badd9183e73296bb`; raw output is 3,113,493 bytes, SHA-256 `6c34800137edad9964606c0acb377435e7af129b69b4c234344d00ed39e2297f`. Complete source and index endpoints remain equal. A separate post-closure census (`246760`, `goal-ninth-repair110-3fe8af6-post-closure-v1.json`) observed no remaining matching Swift, XCTest or formatter process before edits resumed. The original 900-second allowance, 110-case selection and prior failed compile evidence remain intact. All nine completion gates remain open.


### 2026-09-02: carry actual Invoke results through the native provider bridge

The original product criteria are unchanged. The native bridge previously discarded the live source's Boolean Invoke result, allowing an ordinary source refusal to appear as COM success. It now carries that result through the same complete native call and maps ordinary false to `UIA_E_INVALIDOPERATION`, while ordinary true maps to `S_OK`. Existing transport failures take precedence, and the original post-action logical-availability check still runs before either Boolean result is accepted. Disabled elements retain their prior refusal without invoking the action. An action can produce an effect and then return false; no retry, fallback, rollback or fabricated success is introduced.

Two additive C context factories accept optional result callbacks. Both original callback table layouts, original factory signatures and legacy void behavior remain unchanged. The result callback is selected exclusively and invoked once; nonzero raw callback values are Boolean true unless the original call separately records failure. Owned requests use the existing integer reply channel, and the old production void thunk explicitly discards that integer result. Focus, actor-entry checks, nested-request refusal, full-call leases, context adoption/release, wake ownership and timeout policy remain unchanged.

Only two previously approved expectations change in UIANativeRequestTests: its unchanged false Invoke source now returns `.integer(0)` through the owned request, and the same native COM action returns InvalidOperation. All sixteen existing method names, ninety-nine XCTest sites, Focus expectations and exact action ordering remain; the other 677 prior test paths are unchanged. Twelve new async methods contain 31 static configurations and 78 XCTest sites. They cover true/false with and without effects, logical unavailability and placeholders, disabled elements, owned projection failure, an explicitly recorded timeout, close during the original full call, callback selection, original void/nil-result compatibility and nil-table ownership. These are authored headless cases using real C provider vtables, not executed passes or elapsed-time timeout evidence.

Root read the complete private source and independent review at `83bebb415c678d6de69c4ec3a6887fe81dc12eaa`, then independently proved its original forward/reverse trees and its composition at current HEAD. All six canonical preimages remain identical to the packet's base. The C header and C++ source retain checkout line-ending conversion; their normalized bytes equal the exact approved Git postimages, and their original raw preimages are archived. All five Swift postimages are byte-exact. Contracts and strict lint on those five Swift files passed (`b38ba6`), with no formatter changes.

The seven-path source patch is 44,641 bytes, SHA-256 `432011ecc1a1c53fe71815b2d861612b3b335f5dd3bc53fe8b946e88c7915f9c`. The compatibility and testing documents now describe the result contract and its limitations. Including those two documents, the final nine-path patch is 47,926 bytes, SHA-256 `37a5c56c81764ebc31113db5520546f8e7e1d64ecd0925f0a0afc373e26e199a`, staged tree `9a36ee749e4aab5622413c714ea901a7fdcebd65`; `goal-ninth-uia-invoke-result-root-docs-staged-proof-v1.json` records exact old-test preservation. Compilation and focused execution of this combined source remain pending.

This change retains synchronous actor/effect dispatch. It does not satisfy Microsoft's requirement that [Invoke return without blocking and perform the action asynchronously](https://learn.microsoft.com/en-us/windows/win32/api/uiautomationcore/nf-uiautomationcore-iinvokeprovider-invoke). Native-window, worker/modal, allocation-failure and assistive-technology qualification remain separate. The earlier public List passes do not close the still-failing deletion/reinsertion case, the registry interruption, or any other original completion gate.


### 2026-09-02: end the observer-storage write before releasing old captures

The stored scrollObserverStorage setter could release the last old observer payload while its exclusive field write remained active. An observer capture's real destructor then reentered that same setter and triggered the fatal access conflict in the unchanged nil-storage regression. Revoking the old registration receipt in willSet prevented stale cleanup authority, but did not end Swift's field access. This correction addresses that separate lifetime boundary without changing any test expectation.

ViewNode now keeps one private optional backing field and the same internal get/set facade. A non-inlined publication helper pins the previous payload, revokes only its old recorded cleanup claim, and assigns the incoming reference. The backing-field assignment ends before the helper releases its pin. The facade then performs the original registration/unregistration and paint invalidation against the storage and runtime that are current after that synchronous release opportunity. A replacement published by the destructor therefore remains authoritative; the outer write does not restore its original incoming nil or unregister a surviving replacement on that basis.

The helper's reference cannot escape its frame. withExtendedLifetime supplies a minimum lifetime, not forced destruction: other existing owners, including the reconciliation publication helper's separate previous-value pin, can still delay the object's final destruction. No old ownership is removed or replaced. Equal-instance writes still revoke the old cleanup claim and use idempotent registration; nil writes and nested writes retain their normal post-write actions. There is no extra optional field, heap box, observer, history reset, global table, callback queue, or asynchronous work. The changed private reflection label is not a public storage-layout promise; existing reflection tests remain unchanged. Hypothetical new inout/key-path clients are not claimed compiler-qualified by the source census.

The private one-file successor is `0eff3dc009efa79e0550296ded83066c123229be`, based directly on `3fe8af6`. Root read its full 18-addition/6-removal patch and callback contract, independently proved complete forward/reverse trees both at its original base and after the native Invoke bridge commit, and verified exact raw Runtime pre/postimages. The current source tree is `7f0433fe9093b233403d0bb6611ffc79e159c584`; the patch is 2,105 bytes, SHA-256 `b09fc23efc060a4fed18c2ca4321168ab3a8a59cdc31b0100c7b07ad9ed79958`. Contracts and strict lint passed (`a70910`, `3a38a2` / `c12919`) without formatting changes. `goal-ninth-scroll-storage-lifetime-root-staged-proof-v1.json` preserves every one of the 679 prior test paths, including all twenty-nine native-registration methods.

The independent reconciliation of the preceding 110-case run is now retained in `goal-ninth-repair110-3fe8af6-reconciliation-intake-v1`. Its 116,294-byte REPORT.json has SHA-256 `9ccae86707299d35c29c9a4054cd9a8b0a9b706acb218c95099351629ebeb956`. It joins all 110 source methods and 147 case events, finds no duplicate/unknown IDs or orphan terminals, and independently compares all 1,139 captured file records plus index and goal endpoints. Its outcome remains 72 passes, one terminal failure, one started unknown, and 36 unstarted methods, bound only to 3fe8af6. No later source correction rewrites those results.

Compilation and runtime verification of this setter remain pending. The next combined cohort retains all original 110 methods and adds the twelve Invoke-result methods plus the sixteen existing native-request methods. The already identified direct-root Invoke failure is still expected to require its own repair; it is not dropped from the selection. The original nine gates, prior timeouts, actual native 24/27 result, held ordinary-metadata defects and controller/attachment cleanup limits remain open.


### Ninth integration: complete 138-case repair run, three failures retained (2026-09-02)

The complete fixed repair cohort ran at commit `5696695cc4c19f93d3c0411c4be4b09f7f9041d3`, tree `4eb7438124ea98fab63d6e6c06144d0f07378165`. Run `repair138-5696695-8839622b895549a1a4f9f795c51a75f3` ended naturally with direct PowerShell and controller exit 1 after 602.734 seconds, including the reported 446.50-second build. It did not time out or require process termination. All 138 selected XCTest cases started and produced individual terminal outcomes: **135 passed, 3 failed, 0 skipped**. The aggregate XCTest count of 10 assertion failures is not ten failed test cases. The independent fixed-cohort reconciler matched every selected identifier, the Swift Testing zero-case footer, raw log, and complete source/index endpoints. This is focused evidence, not a full-suite pass.

- `RetainedNativeRegistrationTests`: **29/29 pass**, including the previously fatal nil-storage release/reentrant replacement case. The computed storage setter repair now has actual execution evidence for this cohort; this does not qualify the separate retained attachment terminal-publication gap.
- `UIAInvokeResultPropagationTests`: **12/12 pass**; `UIANativeRequestTests`: **16/16 pass**. These exercise the result-preserving C provider bridge and the two explicitly corrected old result expectations, not asynchronous COM Invoke conformance or real native-window qualification.
- `NativeWindowStartupPresentationTests`: **20/20 pass**; `RetainedButtonActionValidationTests`: **17/17 pass**. These do not prove native idle behavior or resolve the prior DashboardUI11/Media152/File14 timeouts.
- `LazyListUIAInvokeCompletionTests`: **10/11 pass**. `testActionDeletionAndAcceptedSameKeyReinsertionDoNotReviveTheOriginalProvider` still fails its seven original assertions. The direct List root and accepted-removal/reinsertion requirements remain unchanged. Root content construction must not inherit a completed row's admission; the production factory-boundary repair is still private and unvalidated.
- `PublicLazyListAccessibilityTests`: **14/15 pass**. `testRealizeAdoptsAndLaysOutTheActualActionTarget` fails at line 94 because native Invoke returns `UIA_E_INVALIDOPERATION` instead of the required `S_OK`. The earlier 15/15 run used the old void-result bridge and did not establish successful runtime Boolean completion. The public success assertion remains unchanged; investigation must explain the false result without another query, a budget reset, or a retry after an effect.
- `WinSwiftUIStepperIntegerBoundsTests`: **17/18 pass**. The pointer case fails only its positive frame width/height assertions at lines 218-219. The integer and overflow cases pass, but pointer geometry and real pointer activation remain unqualified until the fixture/production cause is resolved. Zero-sized geometry must not be skipped or replaced with fake hit testing.

Evidence: raw log 3,959,332 bytes, SHA-256 `d50da3150e525e7e608891424d73833663af75ef63d808fef77d95f26532b9b9`; independent receipt `artifacts/goal-ninth-repair138-5696695-reconciled-v1.json`, 24,912 bytes, SHA-256 `3a503bb9350d809b24df5f3f1cb7396a052a4fd6f1cbccc595f5f46cad46071e`. Reconciler invocation `6623e7/0` records the actual failed cases. After natural direct-child PID 59956 closure, `795dc1/0` recorded no matching compiler, test, renderer, or formatter processes at 07:58:13.8917148 UTC in `goal-ninth-repair138-5696695-post-closure-v1.json`. That is a separate point-in-time census, not continuous descendant or loaded-module attestation. The first ad hoc console summary stopped on the Windows console's Unicode encoding after printing the case results; the independent reconciliation subsequently completed successfully and did not alter the raw run.

All nine original completion gates remain open. No test assertion, time limit, original product requirement, or release criterion was removed; the remaining failures stay in subsequent repair selections. No push or release qualification is authorized by these focused results.


### Ninth integration: resolved Stepper pointer geometry and second dashboard timeout (2026-09-02)

The complete 138-case repair cohort at `5696695` established 135 passes and three failing cases, not a clean gate. One failure was the new Stepper pointer fixture reading its intentionally zero authored frame. The fixture now obtains the placed button rectangle from `RetainedViewRuntime.resolvedLayoutFrame(of:)`, requires both dimensions to be positive, and sends the same real pointer-down/up sequence to that rectangle's midpoint. The original UInt8(254), 0...255 range, step 2, result 255, and complete editing/write/invalidate event assertions are unchanged. Only that method's geometry lookup and its unused authored-frame helper changed; the other 17 methods and all 678 other existing test files remain byte-identical. This is a fixture correction, not a production layout change or an XCTest pass. Strict changed-file lint and contract checks passed; fresh execution remains required. The exact staged source proof is `artifacts/goal-ninth-stepper-pointer-238d265-staged-proof-v1.json`.

The one dashboard retry after the Button validation implementation, at clean `238d265`, also reached its unchanged 900-second bound (observed 900.031 seconds). Its partial 231-byte log records a successful 0.36-second incremental build but no terminal XCTest events. All eleven dashboard interaction results remain UNKNOWN; absent flushed events do not establish that no case executed. The controller returned 124 after its direct PowerShell child exited 1. Root then verified all six previously observed process identities by retained handles, terminated only the still-running owned XCTest process with code 143, observed the other processes exit, and obtained a separate empty process/formatter census. The raw log stayed unchanged (SHA-256 `89de3e66950b974c67f3e89dc0bf072060089d40b1485aa05a1cecddb96f8791`). All complete source/index checkpoint fields matched. See `artifacts/goal-ninth-dashboard-ui11-238d265-timeout-reconciliation-v1.json` and the separately recorded owned-process closure/census.

A single selected-thread diagnostic paused that timed run for 0.578 seconds inside its original timeout, resumed it once, and closed the debugger. Context and stack memory were sampled coherently, but missing unwind information and uncaptured register/stack data limit independent reconstruction of the longer call chain. The observed copy-witness/ownership-publication address associations motivate bounded investigation; they do not prove a complete test-case stack, total runtime cost, or a performance improvement. No further identical retry is justified by this sample alone. Snapshot-controller prelaunch failures likewise produced no render or image and remain recorded separately. All nine original completion gates remain open, with their scope and thresholds unchanged.


### Ninth integration: owned root content is evaluated before component construction (2026-09-02)

The direct-List deletion/reinsertion failure in the 138-case cohort exposed a context boundary, not permission to revive an old row token. A retained row action reinstalls its finite original row context; previously the shared host and independently owned window/document factories evaluated their root values before installing the separate root context. A rejected replacement consequently preserved the old actual tree, so no accepted deletion had occurred. The integrated owned-root slice now brackets root value evaluation with the original native preparation UUID or mounted root descriptor/lookup, copies ordinary environment providers without reading them, and restores the caller's scope. It never obtains replacement authority after the callback. Mounted/document rejection travels through the existing rejected-source marker and journal cleanup, rather than a successful empty root. Original native post-assignment/release checks remain unchanged. Authored work already inside the synchronous native callback may finish after revocation; its returned value and later publication are withheld.

The exact reviewed private source was `259b533`; root integrated its seven paths, applied reviewed formatter-only changes, and added one compatibility-documentation bullet. `artifacts/goal-ninth-owned-root-content-d896bd3-staged-proof-v1.json` records the resulting eight-path source/documentation patch. All original eleven InvokeCompletion test-file bytes and UIA budgets/assertions remain unchanged. Of 679 existing test files at this integration parent, 678 remain exact; the sole approved change routes the shared MountedLazyListTestHost through the production coordinator helper before evaluating content. Eleven new async OwnedRootViewContentTests cover accepted deletion, close/supersession, descriptor lifetime, native default/data-bound factories, document roots, and scope/provider behavior. The native tests use real coordinator/retained-host hooks, not an HWND or platform event loop. They do not add specific capture-destructor-after-assignment coverage. Root strict lint and contract checks passed after formatting; compilation and runtime outcomes are still pending and must be recorded separately.

Legacy factories, direct internal host factory entry, eager declaration-time content, and by-value snapshot argument evaluation are unchanged and remain goal obligations. Existing reopen-during-close and activation-only routes are not redefined; persistent restoration is not added. This change does not repair or claim to repair the separate far-row post-Invoke planning failure. All nine original completion gates remain open and unchanged.


### Ninth integration: preserve unequal corner radii in the public shape path (2026-09-02)

`UnevenRoundedRectangle.path(in:)` now constructs four separate circular corner arcs and their connecting edges instead of substituting the largest radius at every corner. Each radius uses the existing uniform Path constructor's clamp and operand order. Equal radii keep the original ten-element representation; zero/negative extents and nonfinite coordinate behavior are not given a new policy. The already separate retained fill/border route, which preserves four radii and resolves leading/trailing through its build context, is unchanged. The compatibility documentation now distinguishes those routes from the still-uniform clip/content-shape descriptor and legacy consumers.

Thirteen new async regressions cover ordered translated arcs, each named corner, square corners, uniform equivalence, independent clamping, stored nonfinite coordinates, AnyShape forwarding, analytic explicit-Path trimming, and two actual retained-Canvas scene-pixel fixtures. Invalid-coordinate tests inspect values without rasterizing them. Root reviewed the complete source/test change and independent review, integrated the two source paths, applied only two formatter layout changes in the new test file, and updated the existing documentation paragraph. All 680 preexisting test files at parent `ea5cef0` remain byte-identical. Strict lint and architecture contracts passed. The exact three-path staged proof is `artifacts/goal-ninth-uneven-path-ea5cef0-staged-proof-v1.json`; compiler and test results remain pending, not implied by the source review.

Continuous-corner fidelity, public-path RTL qualification, exact uneven clip/content geometry, retained trim at actual bounds, ContainerRelativeShape parent geometry, and InsetShape consistency remain original-scope work. No baseline, visual tolerance, or completion criterion changed. All nine original gates remain open.

Separately, the reviewed V3 snapshot-controller identity correction passed all 74 bounded synthetic controls (`artifacts/goal-ninth-dashboard-snapshot-v3-synthetic-controls-v1.json`, SHA-256 `698068a9131e8124875b9f26e48002596b39a1ace1a5de35c466143a6f313171`). The controls extract pinned scalar predicates and use synthetic metadata; they do not execute controller/helper file I/O, races, process launch, or rendering. Path-versus-handle comparison now requires matching device, inode, file type, size, mtime and birthtime, while full mode/ctime are still compared within each API's own before/after observations. The two prior prelaunch failures remain immutable and produced no image. A real retained snapshot is still required.
