...
$swiftEnvironmentSignature = "$vsDevCmd|$vsArch|$swiftBin|$swiftRuntime|$sdkRoot"
if ($env:SWIFT_WINDOWSUI_DEV_ENV_SIGNATURE -ne $swiftEnvironmentSignature) {
    ...
    $env:SDKROOT = $sdkRoot
    $env:SWIFT_WINDOWSUI_DEV_ENV_SIGNATURE = $swiftEnvironmentSignature
}
